# All In Built-in Browser Runtime Design

## Purpose

All In currently depends on an external Claude Code `/web-access` CDP proxy at `http://localhost:3456` for most browser operations. This makes the project difficult for ordinary users to run outside Claude Code, and the existing Python-side browser connection is not reliable enough in practice.

The goal is to make All In self-contained for browser automation while preserving the working behavior already proven by `/web-access`:

- connect to the user's existing Chrome session and login state;
- keep All In's current `new_tab`, `evaluate`, `click`, `scroll`, `close_tab`, `get_page_info`, and PDF workflow stable;
...
This runtime is not a general web-browsing agent framework. It should not bring over `/web-access` features that do not serve All In directly:
...
- no broad site-pattern framework beyond All In's BOSS直聘 needs.

## Current state

All In's main browser interface is [src/allin/browser/__init__.py](../../../src/allin/browser/__init__.py). It exposes simple functions used by scraping, sending, monitoring, and PDF generation:
...
Most of these functions call `http://localhost:3456`, which is supplied by the external `/web-access` CDP proxy. Only connection checking falls back to `http://localhost:9222/json/version`. The result is that `allin run` can detect Chrome in some cases but still fail when it needs real page operations unless the external proxy is already running.

There is also [src/allin/browser/session.py](../../../src/allin/browser/session.py), which uses Patchright's `connect_over_cdp`, but it is not wired into the current scraping/sending/monitoring flows and does not include the proven `/web-access` fallbacks.

## Recommended architecture

All In should embed a dedicated Node.js CDP runtime adapted from the working `/web-access` runtime, then have the Python browser layer automatically ensure that runtime is available. The implementation should preserve the proven behavior but remove Claude Code-specific skill concerns.
...
Built-in All In CDP Runtime on 127.0.0.1:3456
  ↓ WebSocket CDP
...
This keeps the proven `localhost:3456` API shape but changes ownership: `localhost:3456` becomes All In's own bundled runtime, not Claude Code's skill runtime.

## Runtime files

Add a built-in runtime package under All In:
...
The existing business modules should continue importing from `allin.browser` so the implementation change stays localized.

## Runtime capabilities

The bundled `cdp-proxy.mjs` should preserve these `/web-access` capabilities because they are important for All In reliability:
...
Preserve the endpoint behavior All In already depends on:
...
Also keep the useful `/web-access` operations that support future All In flows:
...
This allows [src/allin/scraper/jobs.py](../../../src/allin/scraper/jobs.py), [src/allin/executor/sender.py](../../../src/allin/executor/sender.py), and [src/allin/executor/monitor.py](../../../src/allin/executor/monitor.py) to keep their current call patterns.

## CLI behavior

`allin connect` should become the user-facing runtime check:

```text
正在检测 Browser Runtime...
✓ Node.js 可用
✓ Chrome 调试端口已发现: 9222
✓ All In CDP Runtime 已启动: http://127.0.0.1:3456
✓ 已连接 Chrome
✓ 发现 BOSS直聘 页面
```
...
`allin run`, `scrape`, `send`, `monitor`, and resume PDF generation should auto-start/check the runtime before browser operations, so users do not need to run `allin connect` manually first.
...
```text
src/allin/browser/runtime/*.mjs
src/allin/browser/runtime/site-patterns/*.md
```

Node.js remains a runtime prerequisite unless a future phase rewrites the proxy in Python. The README should clearly say All In includes its own runtime but still requires Node.js for the bundled CDP proxy.
...
3. Update [src/allin/browser/__init__.py](../../../src/allin/browser/__init__.py) to use the bundled runtime rather than assuming an external proxy.
4. Update [src/allin/ai/resume.py](../../../src/allin/ai/resume.py) PDF generation to use the same runtime helpers instead of hardcoding an external proxy.
...
- `allin connect` with Chrome remote debugging enabled;
- `allin connect` when no Chrome debug port is available;
- `allin scrape --limit 1` or an equivalent low-impact scrape smoke test;
