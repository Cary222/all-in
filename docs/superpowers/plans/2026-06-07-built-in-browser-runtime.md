...
- Create `src/allin/browser/client.py`: small HTTP client around runtime endpoints.
- Create `src/allin/browser/runtime.py`: runtime config, path resolution, Node check, health check, process start, readiness wait.
- Create `src/allin/browser/diagnostics.py`: user-facing diagnostics used by `allin connect`.
- Create `src/allin/browser/runtime/cdp-proxy.mjs`: bundled All In CDP runtime adapted from `/web-access`.
- Create `src/allin/browser/runtime/check-runtime.mjs`: optional standalone runtime check script.
- Create `src/allin/browser/runtime/site-patterns/zhipin.com.md`: bundled BOSS直聘 runtime notes.
- Modify `src/allin/browser/__init__.py`: keep public facade names and delegate to client/runtime.
- Modify `src/allin/config.py`: add browser defaults.
...
- Modify `src/allin/main.py`: make `allin connect` run runtime diagnostics.
- Modify `src/allin/pipeline.py`: show actionable runtime readiness errors.
- Modify `src/allin/ai/resume.py`: use built-in runtime helpers for `/pdf` instead of hardcoded external proxy.
...
### Task 2: Bundle All In CDP runtime files

**Files:**
- Create: `src/allin/browser/runtime/cdp-proxy.mjs`
- Create: `src/allin/browser/runtime/check-runtime.mjs`
- Create: `src/allin/browser/runtime/site-patterns/zhipin.com.md`

- [ ] Copy the proven `/web-access` `cdp-proxy.mjs` into All In runtime.
- [ ] Adapt runtime branding/log messages to `All In Browser Runtime`.
...
**Files:**
- Create: `src/allin/browser/runtime.py`
- Create: `src/allin/browser/client.py`
- Test: `tests/test_browser_runtime.py`

- [ ] Implement runtime path resolution with `importlib.resources.files("allin.browser") / "runtime" / "cdp-proxy.mjs"` and filesystem fallback.
...
**Files:**
- Modify: `src/allin/browser/__init__.py`
...
**Files:**
- Create: `src/allin/browser/diagnostics.py`
- Modify: `src/allin/main.py`
- Modify: `src/allin/pipeline.py`
- Modify: `src/allin/ai/resume.py`
...
- [ ] Update `allin connect` to call diagnostics.
...
```bash
python -m compileall -q "/d/Ai项目/allin/src" "/d/Ai项目/allin/tests"
```
...
```bash
PYTHONPATH="/d/Ai项目/allin/src" python -m unittest discover -s "/d/Ai项目/allin/tests"
```
...
```bash
node --check "/d/Ai项目/allin/src/allin/browser/runtime/cdp-proxy.mjs"
node --check "/d/Ai项目/allin/src/allin/browser/runtime/check-runtime.mjs"
```
...
```bash
PYTHONPATH="/d/Ai项目/allin/src" python -m allin.main connect
```
