...
- Modify `src/allin/ai/prefilter.py`
...
class PrefilterHardGateTests(unittest.TestCase):
    def test_deal_breakers_still_match_title_only(self):
        from allin.ai.prefilter import quick_score
...
```python
    def test_load_config_defaults_to_not_allowing_internships(self):
        from allin.config import load_config
...
class ConfigSchemaTests(unittest.TestCase):
    def setUp(self):
        ...
        self.schema_source = (
            ROOT / "src" / "allin" / "web" / "config_schema.json"
        ).read_text(encoding="utf-8")
...
class ScorerPrefilterTests(unittest.TestCase):
    def setUp(self):
        self.source = (ROOT / "src" / "allin" / "ai" / "scorer.py").read_text(encoding="utf-8")
...
```bash
cd "D:/Ai项目/allin" && PYTHONPATH="D:/Ai项目/allin/src" python -m unittest "D:/Ai项目/allin/tests/test_regression_small_fixes.py" -v
```
...
```bash
git -C "D:/Ai项目/allin" add "tests/test_regression_small_fixes.py"
git -C "D:/Ai项目/allin" commit -m "test: cover hard prefilter gate"
```
...
- [ ] **Step 1: Replace `src/allin/ai/prefilter.py` with hard-gate logic**
...
```bash
cd "D:/Ai项目/allin" && PYTHONPATH="D:/Ai项目/allin/src" python -m unittest \
...
```bash
git -C "D:/Ai项目/allin" add "src/allin/ai/prefilter.py"
git -C "D:/Ai项目/allin" commit -m "feat: hard gate prefilter results"
```
...
**Files:**
- Modify: `src/allin/ai/scorer.py:89-114`
- Test: `tests/test_regression_small_fixes.py`

- [ ] **Step 1: Remove the threshold variable and change the filter condition**

In `src/allin/ai/scorer.py`, replace:
...
```bash
git -C "D:/Ai项目/allin" add "src/allin/ai/scorer.py"
git -C "D:/Ai项目/allin" commit -m "fix: filter only hard prefilter failures"
```
...
**Files:**
- Modify: `src/allin/config.py:37-56`
- Modify: `config.example.yaml:4-28`
- Test: `tests/test_regression_small_fixes.py`

- [ ] **Step 1: Update `src/allin/config.py` defaults**
...
```bash
git -C "D:/Ai项目/allin" add "src/allin/config.py" "config.example.yaml"
git -C "D:/Ai项目/allin" commit -m "config: add internship prefilter default"
```
...
**Files:**
- Modify: `src/allin/web/config_schema.json:7-34`
- Test: `tests/test_regression_small_fixes.py`

- [ ] **Step 1: Add `allow_internship` and remove `prefilter_threshold` in schema**

In `src/allin/web/config_schema.json`, replace the `profile.fields` block:
...
```bash
git -C "D:/Ai项目/allin" add "src/allin/web/config_schema.json"
git -C "D:/Ai项目/allin" commit -m "config: update web schema for hard prefilter"
```
...
**Files:**
- Modify: `src/allin/web/frontend/src/pages/ConfigPage.tsx:103-158`
- Test: `tests/test_regression_small_fixes.py`
...
```bash
git -C "D:/Ai项目/allin" add "src/allin/web/frontend/src/pages/ConfigPage.tsx"
git -C "D:/Ai项目/allin" commit -m "ui: update prefilter settings"
```
...
```bash
cd "D:/Ai项目/allin" && python -m unittest discover -s tests -v
```
...
```bash
git -C "D:/Ai项目/allin" add \
  "tests/test_regression_small_fixes.py" \
  "src/allin/ai/prefilter.py" \
  "src/allin/ai/scorer.py" \
  "src/allin/config.py" \
  "config.example.yaml" \
  "src/allin/web/config_schema.json" \
  "src/allin/web/frontend/src/pages/ConfigPage.tsx"
git -C "D:/Ai项目/allin" commit -m "Implement hard prefilter gate"
```
