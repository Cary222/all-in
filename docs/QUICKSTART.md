...
本指南适合第一次安装和运行 All In。首页只保留最短启动路径，平台登录、浏览器连接和排错细节统一放在这里。
...
```bash
# Windows
chrome.exe --remote-debugging-port=9222 --user-data-dir="%LOCALAPPDATA%\All InChrome"

# macOS
open -na "Google Chrome" --args --remote-debugging-port=9222 --user-data-dir="$HOME/.allin-chrome"

# Linux
google-chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.allin-chrome"
```
