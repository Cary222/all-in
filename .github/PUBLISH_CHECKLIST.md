...
**问题**：项目名包含 "某直聘" 商标
**解决**：
- [x] 对外名称改为 `All In`（中性名称）
- [x] README 免责声明放在最顶部
- [x] 代码中移除 "某直聘" → 改为 "allin"（包名、模块名）
...
### 代码层面
- [x] 全局替换 `bosshunter` → `allin`（包名、import、CLI 名）
...
### 前端层面
- [ ] 构建 production 版本：`cd src/allin/web/frontend && npm run build`
...
| 原名 | 新名 | 位置 |
|------|------|------|
| bosshunter | allin | Python 包名 |
| BossHunter | All In | 项目显示名 |
| bosshunter.db | allin.db | 数据库文件名（旧库路径保留在 `LEGACY_DB_PATH`） |
| `bosshunter` CLI | `allin` CLI | 命令行入口 |
| src/bosshunter/ | src/allin/ | 源码目录 |

> 上游 PR 链接（`github.com/shengjidaguai-china/BossHunter/pull/N`、`github.com/powerycy/BossHunter/pull/N`）保留原名不动，改则断链。
> 平台选择器与数据文件名（`boss.py`、`boss_cities.json`）指的是 BOSS 直聘平台本身，不属于品牌改名范围。
