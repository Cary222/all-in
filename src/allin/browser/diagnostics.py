...
from allin.browser import evaluate, find_boss_tab, find_zhilian_tab, find_job51_tab, find_liepin_tab
from allin.browser.runtime import check_node_available, ensure_runtime, get_runtime_url, runtime_health, runtime_targets
...
def run_browser_diagnostics(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    job51_tab = find_job51_tab() if runtime_ready else None
    liepin_tab = find_liepin_tab() if runtime_ready else None
    browser_product, browser_name = _browser_identity(health)

    errors: list[str] = []
    if not node.get("available") and not runtime_ready:
        errors.append("Node.js is required for All In Browser Runtime.")
    if not runtime_ready:
        if health and health.get("runtime") != "allin":
            errors.append("Runtime port is occupied by a non-All In service.")
        else:
            errors.append("All In Browser Runtime is not ready.")
...
def print_browser_diagnostics(config: dict[str, Any] | None = None, console: Console | None = None) -> bool:
    ...
    if node.get("available"):
        ...
        out.print("  All In 内置浏览器运行时需要 Node.js。请安装 Node.js 22+ 后重试。")

    if result["runtime"]:
        out.print(f"[green]✓[/green] All In CDP Runtime 已启动: {result['runtime_url']}")
    else:
        out.print(f"[red]✗[/red] All In CDP Runtime 未就绪: {result['runtime_url']}")
        if any("non-All In" in error for error in result["errors"]):
            out.print("  端口已被非 All In 服务占用，请停止旧的 CDP Proxy 或在 config.yaml 中修改 browser.proxy_port。")

    if result["chrome"]:
        out.print("[green]✓[/green] 已连接 Chrome")
    elif any("non-All In" in error for error in result["errors"]):
