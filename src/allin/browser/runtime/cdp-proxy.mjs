#!/usr/bin/env node
// All In Browser Runtime - local HTTP-to-CDP bridge for the user's Chrome session.
...
const PORT = parseInt(process.env.ALLIN_BROWSER_PROXY_PORT || process.env.CDP_PROXY_PORT || '3456', 10);
const ENABLE_PORT_GUARD = !['0', 'false', 'no'].includes(String(process.env.ALLIN_ENABLE_PORT_GUARD || 'true').toLowerCase());
const COMMON_PORTS = String(process.env.ALLIN_CHROME_PORTS || '9222,9229,9333')
  .split(',')
  .map((value) => parseInt(value.trim(), 10))
  .filter((value) => value > 0 && value < 65536);
const RUNTIME_NAME = 'allin';
...
if (typeof globalThis.WebSocket !== 'undefined') {
  ...
  try {
    ...
    console.error('[All In Browser Runtime] Node.js < 22 requires ws. Upgrade Node.js or install ws.');
    process.exit(1);
  }
}
      ...
      if (port > 0 && port < 65536 && await checkPort(port)) {
        ...
        const browserName = filePath.includes('Chromium')
          ...
          : filePath.includes('Chrome Canary')
            ...
            : 'Google Chrome';
        console.log(`[All In Browser Runtime] DevToolsActivePort: ${port}${wsPath ? ' with wsPath' : ''}`);
      ...
      if (version?.webSocketDebuggerUrl) {
        ...
        const browserName = product?.startsWith('Edg/')
          ...
          : product?.startsWith('Chrome/')
            ? 'Google Chrome'
            : product || '未知浏览器';
        console.log(`[All In Browser Runtime] Found Chrome debug port: ${port}`);
...
async function connect() {
  ...
  connectingPromise = new Promise((resolve, reject) => {
    ...
    const onOpen = () => {
      ...
      console.log(`[All In Browser Runtime] Connected to Chrome port ${chromePort}`);
      resolve();
    };
    const onError = (event) => {
      cleanup();
      ...
      console.error('[All In Browser Runtime] Connection error:', msg);
      reject(new Error(msg));
    };
    const onClose = () => {
      console.log('[All In Browser Runtime] Chrome connection closed');
      ws = null;
      chromePort = null;
      chromeWsPath = null;
      chromeWsUrl = null;
...
async function main() {
  ...
  if (!available) {
    if (await existingRuntimeHealthy(PORT)) {
      console.log(`[All In Browser Runtime] Existing runtime is running on port ${PORT}`);
      process.exit(0);
    }
    console.error(`[All In Browser Runtime] Port ${PORT} is occupied by another service`);
    process.exit(1);
  }

  server.listen(PORT, '127.0.0.1', () => {
    console.log(`[All In Browser Runtime] Listening on http://127.0.0.1:${PORT}`);
    connect().catch((error) => console.error('[All In Browser Runtime] Initial Chrome connection failed:', error.message));
  });
}

process.on('uncaughtException', (error) => {
  console.error('[All In Browser Runtime] Uncaught exception:', error.message);
});
process.on('unhandledRejection', (error) => {
  console.error('[All In Browser Runtime] Unhandled rejection:', error?.message || error);
});
