#!/usr/bin/env node
// All In Browser Runtime standalone readiness check.
...
const PROXY_PORT = Number(process.env.ALLIN_BROWSER_PROXY_PORT || process.env.CDP_PROXY_PORT || 3456);
...
function startProxyDetached() {
  const logFile = path.join(os.tmpdir(), 'allin-browser-runtime.log');
  const logFd = fs.openSync(logFile, 'a');
  const child = spawn(process.execPath, [PROXY_SCRIPT], {
    detached: true,
    stdio: ['ignore', logFd, logFd],
    env: { ...process.env, ALLIN_BROWSER_PROXY_PORT: String(PROXY_PORT) },
...
async function ensureProxy() {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    if (Array.isArray(result) && await isAllInRuntime()) {
      console.log(`runtime: ready (http://127.0.0.1:${PROXY_PORT})`);
      return true;
    }
