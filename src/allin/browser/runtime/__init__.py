"""Built-in All In Browser Runtime management."""
...
from allin.config import DEFAULTS
...
def _candidate_node_executables() -> list[str]:
    ...
    add(os.environ.get("ALLIN_NODE_PATH"))
...
def is_allin_runtime(config: dict[str, Any] | None = None) -> bool:
    """Return True only when the local service identifies as All In Runtime."""
    health = runtime_health(config)
    return bool(health and health.get("runtime") == "allin")


def runtime_targets(config: dict[str, Any] | None = None) -> list[dict[str, Any]] | None:
    """Return runtime page targets when Chrome and All In Runtime are ready."""
    if not is_allin_runtime(config):
        return None
    try:
...
def _runtime_env(config: dict[str, Any] | None = None) -> dict[str, str]:
    browser = get_browser_config(config)
    env = os.environ.copy()
    env["ALLIN_BROWSER_PROXY_PORT"] = str(browser.get("proxy_port", 3456))
    env["ALLIN_CHROME_PORTS"] = ",".join(str(port) for port in browser.get("chrome_ports", [9222, 9229, 9333]))
    env["ALLIN_ENABLE_PORT_GUARD"] = "true" if browser.get("enable_port_guard", True) else "false"
    return env
...
def ensure_runtime(config: dict[str, Any] | None = None, wait_seconds: float = 15.0) -> bool:
    ...
    needs_fallback = (health is not None and health.get("runtime") != "allin") or (
