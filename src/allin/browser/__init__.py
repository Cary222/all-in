"""Browser facade backed by All In's built-in Browser Runtime."""
...
from allin.browser.client import RuntimeClient
from allin.browser.runtime import ensure_runtime, get_runtime_url, set_browser_config
...
def find_job51_tab() -> dict | None:
    """Find a 51job tab in Chrome without opening or logging in."""
    for target in get_page_targets():
        url = str(target.get("url", ""))
        if "51job.com" in url:
            return target
...
def find_liepin_tab() -> dict | None:
    """Find a Liepin tab in Chrome without opening or logging in."""
    for target in get_page_targets():
        url = str(target.get("url", ""))
