...
def validate_options(
	...
	if target_id:
		result["target_id"] = str(target_id).strip()
	if resume_id:
		result["resume_id"] = str(resume_id).strip()
	return result
...
def _query_jobs(conn: sqlite3.Connection, options: dict[str, Any]) -> list[dict[str, Any]]:
	...
	if options.get("target_id"):
		...
		params.append(options["target_id"])
	if options.get("resume_id"):
		where.append("(resume_id = ? OR matched_resume_id = ?)")
		params.extend([options["resume_id"], options["resume_id"]])
