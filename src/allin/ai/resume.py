...
from allin.browser import close_tab, evaluate, new_tab, print_pdf, screenshot, wait_for_load
from allin.cancellation import OperationCancelled, run_cancellable, stop_requested
from allin.db import get_db

console = Console()

RESUME_COMPLETION_MARKER = "<!-- ALLIN_RESUME_DONE -->"
...
def _render_png_via_cdp(html_content: str, output_path: Path) -> bool:
    ...
    temp_html = Path(tempfile.gettempdir()) / f"allin_resume_{id(output_path)}.html"
    target_id = None
    try:
...
def _render_pdf_via_cdp(html_content: str, output_path: Path) -> bool:
    ...
    temp_html = Path(tempfile.gettempdir()) / "allin_resume.html"
    temp_html.write_text(html_content, encoding="utf-8")
