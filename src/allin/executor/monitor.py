...
from allin.ai.credentials import call_anthropic_text
from allin.browser import new_tab, close_tab, evaluate, click, wait_for_load, get_page_info
from allin.cancellation import (
    OperationCancelled,
    get_stop_event,
    run_cancellable,
    stop_requested,
...
from allin.platform_safety import (
    PlatformAccessGuard,
    PlatformSafetyStop,
    ...
    if job_url:
        for attempt in range(2):
            ...
            if platform == "boss":
                ...
                clicked = click(target_id, "[class*='chat']") or click(target_id, "button[class*='btn']") or click(target_id, "[data-nick='chat']")
            ...
            if clicked:
                ...
                page_url = (info.get("url") or "") if info else ""
                if "chat" in page_url or "im" in page_url or "message" in page_url:
                    return target_id
                if info and any(domain in page_url for domain in ("zhipin.com", "zhaopin.com", "51job.com", "liepin.com")):
                    return target_id
...
def _check_platform_replies(
    platform: str,
    chat_url: str,
    config: dict,
    tracked_jobs: list[dict],
) -> list[dict]:
    """Check replies for a specific non-BOSS platform by navigating to its message center."""
                ...
                if match:
                    ...
                    if job.get("status") == "sent":
                        ...
                        add_history(
                            ...
                            job["id"],
                            "hr_reply_detected",
                            f"{platform} HR回复: {conv.get('last_message', '')[:50]}",
                        )
                    results.append({"job": job, "conversation": conv})
                    console.print(f"[green]  ✓ [{platform}] {job.get('company')} - {job.get('title')} 检测到新回复[/green]")
                    break
    finally:
        db.close()
...
def check_replies(config: dict) -> list[dict]:
    """Check replies across all full-flow platforms."""
    db = get_db()
    try:
        tracked = []
    ...
    boss_tracked = [j for j in tracked if j.get("source_platform") in ("boss", None, "")]
    zhilian_tracked = [j for j in tracked if j.get("source_platform") == "zhilian"]
    job51_tracked = [j for j in tracked if j.get("source_platform") == "51job"]
    liepin_tracked = [j for j in tracked if j.get("source_platform") == "liepin"]

    results = []
    if boss_tracked or not (zhilian_tracked or job51_tracked or liepin_tracked):
        results.extend(_check_boss_replies(config, boss_tracked))
    if zhilian_tracked:
        results.extend(_check_zhilian_replies(config, zhilian_tracked))
    if job51_tracked:
        results.extend(_check_job51_replies(config, job51_tracked))
    if liepin_tracked:
        results.extend(_check_liepin_replies(config, liepin_tracked))
