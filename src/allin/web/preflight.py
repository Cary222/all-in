...
from allin.ai.credentials import (
	get_ai_api_key,
	get_ai_base_url,
...
from allin.browser.diagnostics import run_browser_diagnostics
...
def collect_preflight_checks(mode: str, config: dict, options: dict | None = None) -> list[dict[str, str]]:
	...
	if not ai_required:
		...
		try:
			...
			checks.append(_check("browser_runtime", "浏览器连接", "error", "浏览器连接检测失败", "请重新启动 All In 后再试。", "browser"))
		return checks
	...
	if mode == "full":
		try:
			...
			collection_only = [
				platform for platform in full_options.get("platform_order", [])
			...
			if collection_only:
				...
				checks.append(
					_check(
						...
						"全流程平台能力",
						"error",
						f"{labels}当前只支持单独采集，不能进入发送全流程",
						"请在全流程采集设置中取消勾选仅支持采集的平台。",
						"config",
					)
				)
			else:
				checks.append(
					_check(
						...
						"pass",
						"已选择的平台均接入全流程",
						f"执行顺序：{' → '.join(full_options.get('platform_order', []))}；已选择平台均支持自动投递。",
					)
				)
	return checks
	...
	try:
		...
		return [
			_check(
				...
				f"当前 Python {sys.version.split()[0]}、HTTPX {httpx.__version__}、All In {__version__}；"
	...
	if result.status_code in {404, 405}:
		return [
			_check(
				...
				"warning",
				"接口可访问，但无法自动验证模型",
				"该兼容服务未提供模型列表接口；All In 启动时仍会尝试消息接口。",
				"config",
			)
		]
...
def check_browser_connection(config: dict, collection_options: dict | None = None) -> list[dict[str, str]]:
	...
	if result.get("runtime"):
		...
		checks.append(
			_check(
				...
				"error",
				"缺少可用的 Node.js",
				"All In 无法启动浏览器连接组件，请安装 Node.js 22+ 后重启。",
				"browser",
			)
		)
	else:
		if any("non-All In" in error for error in errors):
			message = "浏览器连接端口被其他程序占用"
			detail = "请停止占用本地 Runtime 端口的程序，或修改 browser.proxy_port 后重启。"
		else:
			message = "All In 浏览器运行组件未启动"
			detail = "请重新启动 All In；若仍失败，运行 allin connect 查看终端诊断。"
	...
	if result.get("chrome"):
		...
		if "zhilian" in selected_platforms:
			...
			if state == "ready":
				checks.append(_check("zhilian_tab", "智联招聘页面", "pass", "智联搜索页可用，未发现登录墙", "已接入投递全流程。"))
				...
				checks.append(_check("zhilian_tab", "智联招聘页面", "warning", "未发现已打开的智联招聘页面", "建议在已连接的 Google Chrome 中打开并登录智联招聘；若未打开，系统将在执行时自动新建标签页。", "browser"))
			else:
				checks.append(_check("zhilian_page", "智联页面结构", "error", "未识别到智联搜索页", str(page_state.get("message") or "请打开智联职位搜索页后重新检查。"), "browser"))
		if "51job" in selected_platforms:
			job51_tab = result.get("job51_tab")
			if job51_tab:
				checks.append(_check("job51_tab", "前程无忧页面", "pass", "已发现前程无忧页面", "已接入投递全流程。"))
			else:
				checks.append(_check("job51_tab", "前程无忧页面", "warning", "未发现已打开的前程无忧页面", "建议在已连接的 Google Chrome 中打开并登录前程无忧；若未打开，系统将在执行时自动新建标签页。", "browser"))
		if "liepin" in selected_platforms:
			liepin_tab = result.get("liepin_tab")
			if liepin_tab:
				checks.append(_check("liepin_tab", "猎聘页面", "pass", "已发现猎聘页面", "已接入投递全流程。"))
			else:
				checks.append(_check("liepin_tab", "猎聘页面", "warning", "未发现已打开的猎聘页面", "建议在已连接的 Google Chrome 中打开并登录猎聘；若未打开，系统将在执行时自动新建标签页。", "browser"))
	return checks
