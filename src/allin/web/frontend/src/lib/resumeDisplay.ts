...
export function resumeOriginalPreviewSrc(
  info: Pick<ResumeInfo, 'id' | 'original_preview_url' | 'cache_buster' | 'uploaded_at'> | null | undefined,
): string {
  const base =
    info?.original_preview_url ||
    (info?.id ? `/api/resumes/${info.id}/original` : '/api/resume/original')
