...
export function ScoreJobsDialog({ open, selectedJobIds, onClose, onStart }: ScoreJobsDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-5">
      <div className="w-full max-w-xl rounded-3xl border border-card-border bg-surface-raised p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          <label className="block text-xs font-bold text-muted">
            评分范围
            <select value={scope} onChange={event => setScope(event.target.value as ScoreScope)} className="mt-1 w-full rounded-xl border border-card-border bg-slate-50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary">
              {Object.entries(scopeLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}
            </select>
          </label>
          <label className="block text-xs font-bold text-muted">
            数量（输入正整数，或输入“全部”）
            <input value={limitText} onChange={event => setLimitText(event.target.value)} className="mt-1 w-full rounded-xl border border-card-border bg-slate-50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
          </label>
          {scope === 'selected' && <div className="rounded-xl bg-primary/10 px-3 py-2 text-sm text-primary">岗位池已选择 {selectedJobIds.length} 条</div>}
          {preview && (
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-700">
          ...
          {runs.filter(run => ['running', 'paused'].includes(run.status)).slice(0, 3).map(run => (
            <div key={run.id} className="rounded-2xl border border-card-border bg-slate-50 p-3 text-sm">
          ...
          {message && <div className="rounded-xl bg-primary/10 px-3 py-2 text-sm text-primary">{message}</div>}
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-xl border border-card-border bg-slate-50 p-3"><div className="text-xs text-muted">{label}</div><div className="mt-1 text-xl font-black text-primary">{value}</div></div>
}
