...
import { PLATFORM_LABELS, PLATFORM_SHORT_LABELS, platformSupportsFullFlow } from '@/lib/platforms'
...
export function CollectJobsDialog({ open, mode = 'collect', activeTask, onClose, onStart }: CollectJobsDialogProps) {
  ...
  useEffect(() => {
    ...
    fetch('/api/config', { cache: 'no-store' })
      ...
      .then(config => {
        ...
        if (mode === 'full') {
          if (!platformSupportsFullFlow('zhilian')) nextZhilian.enabled = false
          if (!platformSupportsFullFlow('51job')) nextJob51.enabled = false
          if (!platformSupportsFullFlow('liepin')) nextLiepin.enabled = false
        }
        ...
        const fullEligibleOrder = nextOrder.filter(platform => platformSupportsFullFlow(platform))
        setDrafts({ boss: nextBoss, zhilian: nextZhilian, '51job': nextJob51, liepin: nextLiepin })
        setOrder(mode === 'full' ? (fullEligibleOrder.length ? fullEligibleOrder : ['boss']) : (nextOrder.length ? nextOrder : ['boss']))
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" role="dialog" aria-modal="true" aria-label="岗位采集">
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-card-border bg-surface-raised p-6 shadow-2xl">
        ...
        {mode === 'collect' && <div className="mt-4 rounded-2xl border border-card-border bg-slate-50 p-4">
          ...
          <div className="mt-4 rounded-2xl border border-primary/20 bg-primary/10 p-4">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {Object.entries(activeTask.progress.platforms).map(([platform, state]) => (
                <div key={platform} className="rounded-xl border border-card-border bg-surface-raised p-3 text-sm">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {(['boss', 'zhilian', '51job', 'liepin'] as PlatformId[]).map(platform => {
            ...
            return (
              <section key={platform} className={`rounded-2xl border p-4 ${draft.enabled ? 'border-primary/30 bg-slate-50' : 'border-card-border bg-surface-raised opacity-70'}`}>
                <div className="flex items-center justify-between gap-3">
                  <label className="flex items-center gap-2 text-lg font-black"><input type="checkbox" checked={draft.enabled} disabled={mode === 'full' && !platformSupportsFullFlow(platform)} onChange={event => togglePlatform(platform, event.target.checked)} className="h-4 w-4 accent-primary" />{label}</label>
                ...
                {draft.enabled && <div className="mt-4 space-y-3">
                  ...
                  {platform !== 'boss' ? <>
                    ...
                    <div className="rounded-xl border border-card-border bg-surface-raised px-3 py-2 text-xs text-muted">
                  ...
                  </> : <p className="rounded-xl border border-card-border bg-surface-raised px-3 py-2 text-xs text-muted">BOSS 城市编码由系统内置匹配，无需填写。</p>}
                ...
                {!draft.enabled && mode === 'full' && !platformSupportsFullFlow(platform) && <p className="mt-3 text-xs text-muted">当前只支持“岗位采集”，不进入发送全流程。</p>}
              </section>
            )
          })}
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-card-border bg-slate-50 p-4">
          <div className="flex items-center justify-between gap-3"><div><div className="text-sm font-black">执行顺序</div><p className="mt-1 text-xs text-muted">{mode === 'full' ? '四平台均已接入全流程，按队列顺序串行执行采集、评分、开聊/投递与回复监测。' : '各平台按队列顺序串行执行职位采集与更新。'}</p></div><div className="flex gap-2">{enabledOrder.map((platform, index) => <div key={platform} className="flex items-center gap-1 rounded-full bg-surface-raised px-3 py-1 text-xs font-black text-primary"><span>{index + 1}. {PLATFORM_SHORT_LABELS[platform]}</span><button type="button" onClick={() => move(platform, -1)} disabled={index === 0} aria-label="上移"><ArrowUp className="h-3 w-3" /></button><button type="button" onClick={() => move(platform, 1)} disabled={index === enabledOrder.length - 1} aria-label="下移"><ArrowDown className="h-3 w-3" /></button></div>)}</div></div>
        </div>

        <label className="mt-4 flex items-center justify-between rounded-2xl border border-card-border bg-surface-raised p-4"><div><div className="text-sm font-black">{mode === 'full' ? '全流程自动评分' : '采集后自动评分'}</div><p className="mt-1 text-xs leading-5 text-muted">{mode === 'full' ? '全流程必须先评分；评分后进入人工确认，再按平台适配器执行招呼和监测。' : '默认关闭；开启后只评分本轮新增岗位，评分结束即停止，不发送消息、不投递、不监测。'}</p></div><Switch checked={mode === 'full' || autoScore} onChange={mode === 'full' ? () => undefined : setAutoScore} disabled={mode === 'full'} /></label>
