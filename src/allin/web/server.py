"""All In Web Server - Bottle HTTP service + API routes.
...
from allin import __version__
from allin.ai.credentials import AIRequestError, get_ai_api_key, list_ai_models
from allin.ai.scorer import sanitize_score_trace
from allin.cities import CityRefreshError, get_city_map, load_city_snapshot, refresh_city_cache
...
from allin.collection.capabilities import PLATFORM_DISPLAY_NAMES, platform_supports
from allin.collection.orchestrator import CollectionOrchestrator, normalize_collection_options
from allin.collection.platforms.zhilian import load_zhilian_city_snapshot
from allin.collection.platforms.job51 import load_51job_city_snapshot
from allin.collection.platforms.liepin import load_liepin_city_snapshot
...
from allin.job_filters import parse_monthly_salary_k
from allin.job_export import InvalidJobSelectionError, export_jobs, export_row_count
from allin.scoring_run_store import (
	create_scoring_run,
	get_scoring_run,
	list_scoring_runs,
...
from allin.web.preflight import check_ai_connection, collect_preflight_checks, error_messages
...
from allin.web.resume_names import resolve_active_resume_path, select_resume_markdown_filename
...
from allin.web.resume_upload import ResumeUploadError, prepare_resume_content
from allin.web.city_lookup import CityLookupError, lookup_city
...
def set_base_dir(base_dir: Path | str) -> None:
	...
	RESUME_DIR = DATA_DIR / "resumes"
	CONFIG_PATH = BASE_DIR / "config.yaml"
	# Adopt pre-rename data before any run bookkeeping reads an empty database.
	adopt_legacy_db(DATA_DIR / "allin.db")
	mark_orphaned_scoring_runs_paused(DATA_DIR / "allin.db")
	mark_orphaned_collection_runs_stopped(DATA_DIR / "allin.db")


def _get_web_db():
	"""Open the dashboard database from the resolved runtime data directory."""
	return get_db(DATA_DIR / "allin.db")
...
def _preflight_messages(mode: str, config: dict, options: dict | None = None) -> list[str]:
	...
	if mode == "collect":
		...
		if collection_options.get("auto_score"):
			resumes = list_resumes(config)
			resume_path = config.get("profile", {}).get("resume_path", "")
			has_resume = bool(resumes) or (bool(resume_path) and Path(str(resume_path)).exists())
			if not has_resume:
	...
	resumes = list_resumes(config)
	has_resume = bool(resumes) or (bool(resume_path) and Path(str(resume_path)).exists())
	if mode in {"full", "rescore"} and not has_resume:
...
def _execute_collect(task: WorkbenchTask, config: dict) -> None:
	...
	if "_collection_options" not in config:
		...
		from allin.ai.scorer import score_jobs
		from allin.scraper.jobs import scrape_jobs
...
def _execute_monitor(task: WorkbenchTask, config: dict, *, initial_cooldown: bool = False) -> None:
	...
	if _stop_for_active_platform_lock(task, platform="boss"):
		return
	...
	try:
		...
		from allin.executor.monitor import close_monitor_chat_target
...
def _execute_deliver(task: WorkbenchTask, config: dict) -> None:
	...
	delivery_platform = config.get("_platform") or "boss"
	if _stop_for_active_platform_lock(task, platform=delivery_platform):
		return
	queue_lock = task.context.setdefault("delivery_queue_lock", Lock())
	with queue_lock:
	...
	try:
		while not task.stop_requested.is_set():
			...
			if "platform" in batch:
				if batch["platform"]:
					current_config["_platform"] = batch["platform"]
				else:
					current_config.pop("_platform", None)
			if batch.get("direct_send"):
...
def _stop_for_active_platform_lock(task: WorkbenchTask, platform: str | None = None) -> bool:
	db = _get_web_db()
	try:
		lock = get_active_platform_safety_lock(db, platform=platform)
	finally:
		db.close()
	if not lock:
		return False
	p_key = lock.get("platform", platform or "平台")
	p_labels = {"boss": "BOSS 直聘", "zhilian": "智联招聘", "51job": "前程无忧", "liepin": "猎聘"}
	p_name = p_labels.get(p_key, p_key)
	reason = f"为了账户安全，{p_name} 风险冷却尚未结束，已停止本次访问"
	task.stop_reason = reason
	task.stop_requested.set()
	_log(task, reason)
	return True
...
def _execute_deliver_batch(task: WorkbenchTask, config: dict) -> None:
	from allin.ai.greeter import generate_greetings
	...
	if not config.get("_workbench_skip_greeting"):
		_log(task, "生成招呼语")
		generated_count = generate_greetings(config, job_ids=selected_job_ids or None, db_path=DATA_DIR / "allin.db")
		...
		if selected_job_ids and ready_count < len(selected_job_ids):
			...
			_log(
				task,
				f"{missing_count} 个岗位未生成招呼语，已保留为待生成，可在对应招聘平台中手动填写；"
				"其余岗位继续进入发送流程。",
			)
	_log(task, "发送招呼语")
	...
	sent_count = send_greetings(config, force=False, db_path=DATA_DIR / "allin.db")
...
def _execute_greet(task: WorkbenchTask, config: dict) -> None:
	from allin.ai.greeter import _get_resume_summary, generate_greetings
...
def api_history_open_chat(history_id):
	with job_mutation_lock:
		...
		from allin.executor.monitor import MonitorRiskDetected, _open_conversation_from_chat_list
...
def api_history_prepare_reply(history_id):
	with job_mutation_lock:
		...
		from allin.executor.monitor import process_detected_reply
...
def _scoring_options_from_body(body: dict) -> dict:
	...
	options = validate_options(
		scope=raw_options.get("scope", "pending"),
		limit=raw_options.get("limit"),
		job_ids=raw_options.get("job_ids", []),
		force_rescore=raw_options.get("force_rescore", False),
		target_id=raw_options.get("target_id"),
		resume_id=raw_options.get("resume_id"),
	)
...
def api_scoring_start():
	...
	try:
		...
		if options.get("target_id"):
			stored_options["target_id"] = options["target_id"]
		if options.get("resume_id"):
			stored_options["resume_id"] = options["resume_id"]
...
def api_scoring_runs():
	return _json_response(list_scoring_runs(DATA_DIR / "allin.db"))
...
def _resolve_collection_resume(mode: str, options: dict | None) -> dict | None:
	if options and options.get("resume_run_id"):
		...
		return boss_resume_options(DATA_DIR / "allin.db", options["resume_run_id"])
	return options
...
def api_workbench_task_start():
	try:
		...
		if mode == "collect":
			...
			if collection_only:
				labels = "、".join(PLATFORM_DISPLAY_NAMES.get(p, p) for p in collection_only)
				return _json_response({
					"error": f"{labels}当前只支持单独采集，不能进入发送全流程",
			...
			for platform, value in collection_options["platforms"].items():
				...
				platform_configs[platform] = {
					**existing,
					...
					# Merge instead of replace: the collection dialog only submits the
					# fields it edits, so a wholesale replacement would silently drop
					# platform search settings it does not know about (e.g.
					# resume_ttl_hours) and the saved value would appear to revert.
					"search": {**existing_search, **value},
				}
...
def api_collection_runs():
	try:
		...
		return _json_response(list_collection_runs(DATA_DIR / "allin.db", limit=limit))
	except (TypeError, ValueError) as exc:
		return _json_response({"error": str(exc)}, 400)
...
def api_collection_run_detail(run_id):
	run = get_collection_run(DATA_DIR / "allin.db", run_id)
	if not run:
...
def api_workbench_deliver():
	try:
		...
		with job_mutation_lock:
			...
			target_platforms = {str(row["source_platform"] or "boss") for row in platform_rows}
			deliver_platform = next(iter(target_platforms)) if len(target_platforms) == 1 else None

			status = task_runner.status(platform=deliver_platform)
			...
			if active_task and not waiting_task and not monitoring_task and not delivery_task:
				# Allow independent platform worker slots to run in parallel
				if (
					active_runtime_task
					and active_runtime_task.slot != "global"
					and active_runtime_task.mode not in {"full", "score", "rescore"}
					and deliver_platform
					and active_runtime_task.platform
					and active_runtime_task.platform != deliver_platform
				):
					pass  # Different platform slot, no conflict
				else:
					has_conflict = True
			...
			if deliver_platform:
				deliver_options["_platform"] = deliver_platform
			if direct_send:
...
def api_job_outreach_resume_save(job_id):
	...
	try:
		from allin.ai.resume import save_resume_draft
...
def _api_history_reply_locked(history_id):
	...
	try:
		...
		from allin.executor.monitor import _build_reply_resolution_detail
...
def api_history_resume_retry(history_id):
	...
	try:
		...
		from allin.ai.resume import generate_tailored_resume, get_last_resume_failure_reason
...
def api_resumes_list():
	...
	try:
		config = load_config(CONFIG_PATH)
		items = list_resumes(config, BASE_DIR, CONFIG_PATH)
		return _json_response({"resumes": items})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/resumes/upload", method="POST")
def api_resumes_upload():
	"""Upload and index a new resume file."""
	try:
		upload = request.files.get("file")
		if not upload:
			return _json_response({"error": "No file uploaded"}, 400)
		...
		target_direction = request.forms.get("target_direction") or ""
		is_default_val = request.forms.get("is_default")
		is_default = str(is_default_val).lower() in ("true", "1", "yes")

		content = upload.file.read()
		raw_name = upload.raw_filename or upload.filename
		config = load_config(CONFIG_PATH)

		new_meta = save_resume_file(
			content,
			raw_name,
			name=name,
		...
		return _json_response({"success": True, **new_meta})
	except ResumeUploadError as e:
		return _json_response({"error": str(e)}, 400)
	except Exception as e:
		return _json_response({"error": str(e)}, 500)
...
def api_resume_get_one(resume_id):
	"""Get single resume metadata and Markdown content."""
	try:
		config = load_config(CONFIG_PATH)
		meta, content = get_resume_by_id(resume_id, config, BASE_DIR)
		if not meta:
			return _json_response({"error": "未找到简历"}, 404)
		return _json_response({**meta, "content": content})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/resumes/<resume_id>", method="PUT")
def api_resume_update(resume_id):
	"""Update resume metadata (name, direction, is_default)."""
	try:
		...
		return _json_response({"success": True, **updated})
	except KeyError:
		return _json_response({"error": "未找到简历"}, 404)
	except Exception as e:
		return _json_response({"error": str(e)}, 500)
...
@app.route("/api/resumes/<resume_id>", method="DELETE")
def api_resume_delete_one(resume_id):
	"""Delete a specific resume and its files."""
	try:
		config = load_config(CONFIG_PATH)
		ok = delete_resume_by_id(resume_id, config, BASE_DIR, CONFIG_PATH)
		if not ok:
			return _json_response({"error": "未找到简历或删除失败"}, 404)
		return _json_response({"success": True})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/resumes/<resume_id>/original")
def api_resume_original_by_id(resume_id):
	"""Serve companion original PDF for a specific resume."""
	try:
		config = load_config(CONFIG_PATH)
		meta, _ = get_resume_by_id(resume_id, config, BASE_DIR)
		if not meta or not meta.get("path"):
			abort(404, "Resume not found")
		pdf_path = companion_pdf_path(Path(meta["path"]))
		if not pdf_path.is_file():
			abort(404, "Original PDF not found")
		return static_file(pdf_path.name, root=str(pdf_path.parent), mimetype="application/pdf")
	except HTTPResponse:
...
@app.route("/api/job-targets")
def api_job_targets_get():
	"""Get all configured job targets."""
	try:
		config = load_config(CONFIG_PATH)
		targets = get_job_targets(config)
		return _json_response({"targets": targets})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/job-targets", method="POST")
def api_job_targets_save():
	"""Save or replace full job targets list."""
	try:
		payload = request.json or {}
		targets_data = payload.get("targets")
		if targets_data is None and isinstance(payload, list):
			targets_data = payload
		if not isinstance(targets_data, list):
			return _json_response({"error": "targets 必须为数组"}, 400)
		config = load_config(CONFIG_PATH)
		normalized = normalize_job_targets(targets_data)
		config["job_targets"] = normalized
		save_config(config, CONFIG_PATH)
		return _json_response({"success": True, "targets": normalized})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/job-targets/<target_id>", method="PUT")
def api_job_target_update(target_id):
	"""Update a specific job target."""
	try:
		payload = request.json or {}
		config = load_config(CONFIG_PATH)
		targets = get_job_targets(config)
		...
		for t in targets:
			if t.get("id") == target_id:
				...
				break
		if not found:
			return _json_response({"error": "未找到求职目标"}, 404)
		config["job_targets"] = normalize_job_targets(targets)
		save_config(config, CONFIG_PATH)
		updated_target = next((t for t in config["job_targets"] if t["id"] == target_id), None)
		return _json_response({"success": True, "target": updated_target})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/job-targets/<target_id>", method="DELETE")
def api_job_target_delete(target_id):
	...
	try:
		...
		if len(remaining) == len(targets):
			return _json_response({"error": "未找到求职目标"}, 404)
		config["job_targets"] = normalize_job_targets(remaining)
		save_config(config, CONFIG_PATH)
		return _json_response({"success": True})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)
...
# ─── Multi-Platform Safety & Cooldown APIs ───────────────

@app.route("/api/safety/status")
def api_safety_status():
	"""Get current safety lock status and today's access count for all platforms."""
	try:
		db = _get_web_db()
		try:
			status = get_all_platform_safety_status(db)
		finally:
			db.close()
		return _json_response({"platforms": status})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)


@app.route("/api/safety/unlock", method="POST")
def api_safety_unlock():
	"""Manually clear safety lock cooldown for a specific platform or all platforms."""
	try:
		payload = request.json or {}
		platform = payload.get("platform", "all")
		db = _get_web_db()
		try:
			cleared_count = clear_platform_safety_lock(db, platform=platform)
			new_status = get_all_platform_safety_status(db)
		finally:
			db.close()
		return _json_response({
			"success": True,
			"cleared_count": cleared_count,
			"cleared_platform": platform,
			"platforms": new_status,
		})
	except Exception as e:
		return _json_response({"error": str(e)}, 500)
...
def run_server(host: str = "127.0.0.1", port: int = 8686, open_browser: bool = True):
	...
	if not (FRONTEND_DIR / "index.html").is_file():
		raise SystemExit(
			"前端资源未构建：请在 src/allin/web/frontend 下执行 "
