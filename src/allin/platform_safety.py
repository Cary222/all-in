...
class PlatformAccessGuard:
    ...
    def _get_safety_config(self) -> dict:
        global_cfg = dict(self.config.get("safety", {})) if isinstance(self.config.get("safety"), dict) else {}
        platform_cfg = self.config.get("platforms", {}).get(self.platform, {}) if isinstance(self.config.get("platforms"), dict) else {}
        if isinstance(platform_cfg, dict) and isinstance(platform_cfg.get("safety"), dict):
            global_cfg.update(platform_cfg["safety"])
        return global_cfg

    def ensure_unlocked(self) -> None:
        lock = get_active_platform_safety_lock(self.conn, platform=self.platform)
        if lock:
    ...
    def lock(self, reason: str, *, minutes: int | None = None) -> None:
        ...
        set_platform_safety_lock(self.conn, reason, minutes=lock_minutes, platform=self.platform)
