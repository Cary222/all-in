...
from allin.config import load_config, migrate_legacy_credentials

console = Console()
GITHUB_URL = "https://github.com/Cary222/all-in"


def _hint_web():
    """Print a one-line hint about the web dashboard."""
    console.print("[dim]💡 运行 allin web 可打开可视化看板[/dim]")
...
def _hint_star_support(base_dir: Path | None = None):
    ...
    if marker.exists():
        return
    console.print(f"[dim]⭐ 如果 All In 帮你节省了求职时间，欢迎 Star 支持继续维护：{GITHUB_URL}[/dim]")
    try:
...
def _prompt_setup(port: int = 8686) -> None:
    ...
    console.print()
    console.print("[bold cyan]═══ 欢迎使用 All In ═══[/bold cyan]")
    console.print()
    console.print("[yellow]检测到尚未配置，建议先进入 Web 端完成初始设置：[/yellow]")
    console.print()
    console.print(f"  [bold]allin web[/bold]  →  打开配置面板 (http://127.0.0.1:{port})")
    console.print()
...
@click.group(name="allin", invoke_without_command=True)
@click.version_option(version=__version__, prog_name="allin")
@click.option("--config", "config_path", default=None, type=click.Path(exists=False), help="配置文件路径（默认 config.yaml）")
@click.pass_context
def cli(ctx: click.Context, config_path: str | None) -> None:
    """All In - 多平台智能求职 Agent"""
    ...
    from allin.browser import configure
    ...
    if ctx.invoked_subcommand is None:
        if _is_first_run(path):
            ...
            console.print("[bold cyan]All In[/bold cyan] 已就绪")
            console.print("[dim]运行 allin --help 查看可用命令[/dim]")
            _hint_web()
...
def connect(ctx: click.Context) -> None:
    """检测 All In Browser Runtime 与 Chrome 连接"""
    from allin.browser import configure
    from allin.browser.diagnostics import print_browser_diagnostics
...
def ai_status(ctx: click.Context) -> None:
    ...
    from allin.web.preflight import check_ai_connection
...
def confirm(ctx: click.Context) -> None:
    """展示投递清单并确认"""
    from allin.ui.confirm import show_confirmation
...
def status(ctx: click.Context, full: bool) -> None:
    """查看投递状态统计"""
    if full:
        from allin.tracker.status import show_dashboard
        show_dashboard()
    else:
        from allin.tracker.status import show_status
        show_status()
...
def run(ctx: click.Context) -> None:
    ...
    from allin.pipeline import run_pipeline

    config = ctx.obj["config"]
    console.print("[bold cyan]═══ All In 启动 ═══[/bold cyan]\n")
    run_pipeline(config)
    _hint_web()
    _hint_star_support()
...
def resume(ctx: click.Context, job_id: str | None) -> None:
    """为指定岗位生成定制简历PDF"""
    from allin.ai.resume import generate_tailored_resume
...
def web(ctx: click.Context, port: int, no_open: bool) -> None:
    ...
    from allin.web.server import run_server, set_base_dir

    set_base_dir(ctx.obj["base_dir"])
    console.print("[bold cyan]═══ All In Web Dashboard ═══[/bold cyan]")
