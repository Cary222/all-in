...
from allin.collection.models import (
    CollectionProgress,
    JobCandidate,
    PlatformCollectionRequest,
    PlatformCollectionResult,
)
from allin.collection.platforms.boss import BossCollector, normalize_boss_search_filters
from allin.collection.platforms.job51 import Job51Collector, get_51job_city_code
from allin.collection.platforms.liepin import LiepinCollector, get_liepin_city_code
...
def normalize_collection_options(config: dict[str, Any], raw_options: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    target_id = str(supplied.get("target_id") or "").strip()
    target_name = str(supplied.get("target_name") or "").strip()
    resume_id = str(supplied.get("resume_id") or "").strip()
    if target_id and not target_name:
        from allin.config import get_job_targets
        all_targets = get_job_targets(config)
...
def validate_collection_options(options: dict[str, Any]) -> dict[str, Any]:
    ...
    if options.get("target_id"):
        normalized["target_id"] = str(options["target_id"]).strip()
        normalized["target_name"] = str(options.get("target_name") or "").strip()
        normalized["resume_id"] = str(options.get("resume_id") or "").strip()
...
class _SharedProcessor:
    ...
    def save(self, candidate: JobCandidate) -> bool:
        ...
        if matching_deal_breaker(candidate.jd, profile.get("jd_deal_breakers") or []):
            ...
            return True
        if not candidate.target_id and self.request.target_id:
            candidate.target_id = self.request.target_id
        if not candidate.target_name and self.request.target_name:
            candidate.target_name = self.request.target_name
        if not candidate.resume_id and self.request.resume_id:
            candidate.resume_id = self.request.resume_id
        try:
...
class CollectionOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.db_path = db_path or Path("./data/allin.db")
        ...
        try:
            for index, platform in enumerate(order, start=1):
                if self.stop_event is not None and self.stop_event.is_set():
                    ...
                    break
                raw = dict(options["platforms"][platform])
                if "target_id" not in raw and options.get("target_id"):
                    raw["target_id"] = options.get("target_id")
                    raw["target_name"] = options.get("target_name")
                    raw["resume_id"] = options.get("resume_id")
