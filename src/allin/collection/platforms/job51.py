...
from allin.browser import close_tab, evaluate, get_page_targets, new_tab
from allin.collection.base import CollectionBlockedError, CollectorHooks
from allin.collection.checkpoint import resolve_resume_ttl_hours
from allin.collection.models import JobCandidate, PlatformCollectionRequest, PlatformCollectionResult
...
from allin.throttle import SendWindowChecker, should_take_day_off
...
class Job51Collector:
    ...
    def _resume_ttl_hours(self) -> int:
        """断点续采有效期（小时）；0 表示停用词级断点。"""
        return resolve_resume_ttl_hours(self.config, "51job")
    ...
    def collect(
        ...
        # 反检测前置：时间窗口 + 随机休息日（复用 All In throttle 配置）
            ...
            for city in request.cities:
                ...
                for kw in request.keywords:
                    ...
                    if (city, kw) in collected_combos:
                        self._log(hooks, f"51job {kw} 断点续采：{self._resume_ttl_hours()}h 内已完成，整词跳过",
    ...
    def _collect_keyword(
        ...
        *,
        ...
        saved_page = (
            get_page_progress(self.safety_conn, "51job", city, kw, within_hours=self._resume_ttl_hours())
            if self.safety_conn is not None
            else 0
        )
