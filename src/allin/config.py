"""Configuration loader for All In."""
...
def _normalize_config_sections(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for section, defaults in DEFAULTS.items():
        ...
        elif isinstance(defaults, list) and not isinstance(config.get(section), list):
            config[section] = _deep_copy_dict(defaults)

    if "job_targets" in config and isinstance(config["job_targets"], list):
        config["job_targets"] = normalize_job_targets(config["job_targets"])

    return remove_retired_collection_settings(config)


def normalize_job_targets(targets: Any) -> list[dict[str, Any]]:
    """Ensure a list of job targets matches the expected schema."""
    ...
    for idx, item in enumerate(targets):
        ...
        name = str(item.get("name") or f"求职目标 {idx + 1}").strip()
        enabled = bool(item.get("enabled", True))
        resume_id = str(item.get("resume_id") or "default").strip()
        keywords = [str(k).strip() for k in item.get("keywords", []) if str(k).strip()]
        cities = [str(c).strip() for c in item.get("cities", []) if str(c).strip()]
        city_codes = item.get("city_codes", {}) if isinstance(item.get("city_codes"), dict) else {}
        salary_min = int(item.get("salary_min", 0)) if str(item.get("salary_min", 0)).isdigit() else 0
        salary_max = int(item.get("salary_max", 0)) if str(item.get("salary_max", 0)).isdigit() else 0
        extra_highlights = str(item.get("extra_highlights") or "").strip()
        greeting_preference = str(item.get("greeting_preference") or "").strip()
...
def get_job_targets(config: dict[str, Any]) -> list[dict[str, Any]]:
    """Return configured job targets or generate a fallback from profile & search."""
    targets = config.get("job_targets")
    if isinstance(targets, list) and targets:
        return normalize_job_targets(targets)

    # Fallback to single legacy target
    profile = config.get("profile", {})
    search = config.get("search", {})
    keywords = list(search.get("keywords") or [])
    cities = list(search.get("cities") or profile.get("target_cities") or [])
    return [{
        "id": "target_default",
        "name": "默认求职目标",
        "enabled": True,
        ...
        "keywords": keywords,
        "cities": cities,
        "city_codes": dict(search.get("city_codes") or {}),
        "salary_min": int(profile.get("salary_min", 0) or 0),
        "salary_max": int(profile.get("salary_max", 0) or 0),
        "extra_highlights": str(profile.get("extra_highlights") or ""),
        "greeting_preference": str(profile.get("greeting_preference") or ""),
    }]
