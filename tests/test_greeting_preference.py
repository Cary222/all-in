...
from allin.ai import greeter
from allin.ai.greeter import _generate_greeting_once
...
def test_greeting_preference_is_bounded_and_cannot_replace_fixed_rules():
    ...
    with patch("allin.ai.greeter._call_claude", side_effect=fake_call):
...
def test_graduation_context_uses_education_end_year_as_class_year():
    ...
    with patch("allin.ai.greeter.date", FixedDate):
