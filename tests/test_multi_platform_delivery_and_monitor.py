...
class LiepinGreetingOnceTests(TestCase):
    ...
    def test_liepin_already_applied(self, mock_new_tab, mock_eval, mock_wait, mock_close):
        job = {"id": "liepin:1", "url": "https://www.liepin.com/job/1.shtml", "greeting": "Hello"}
        mock_eval.return_value = json.dumps({
            "success": True,
            "already_sent": True,
            "verified": True,
            "history_detail": "岗位已处于已应聘状态",
        })
        res, target = _send_liepin_greeting_once(job, "Hello", {"browse_before_greet": False})
        self.assertTrue(res["success"])
        self.assertTrue(res.get("already_sent"))
        self.assertIn("a.btn-main", mock_eval.call_args.args[1])
        self.assertIn("继续聊", mock_eval.call_args.args[1])
