...
class TestCheckpointsDisabled:
    """within_hours=0 停用断点：即使是刚写入的断点也必须被忽略。"""

    def test_zero_disables_word_checkpoint(self, db_conn: sqlite3.Connection) -> None:
        mark_combo_collected(db_conn, "liepin", "北京", "python")
        assert get_collected_combos(db_conn, "liepin", within_hours=24) == {("北京", "python")}
        assert get_collected_combos(db_conn, "liepin", within_hours=0) == set()

    def test_zero_disables_page_checkpoint(self, db_conn: sqlite3.Connection) -> None:
        upsert_page_progress(db_conn, "liepin", "北京", "python", 4)
        assert get_page_progress(db_conn, "liepin", "北京", "python", within_hours=24) == 4
        assert get_page_progress(db_conn, "liepin", "北京", "python", within_hours=0) == 0