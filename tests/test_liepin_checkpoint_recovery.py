...
from allin.collection.platforms.liepin import LiepinBrowser, LiepinCollector
from allin.db import get_db, get_collected_combos, get_page_progress, mark_combo_collected, upsert_page_progress
...
def safe_timing():
    with patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=True), patch("allin.collection.platforms.liepin.should_take_day_off", return_value=False):
        yield
...
def test_failed_database_save_must_not_mark_combo_complete(tmp_path):
    ...
    with patch("allin.collection.orchestrator.LiepinCollector", side_effect=factory), patch("allin.collection.orchestrator.insert_job_if_new", side_effect=RuntimeError("simulated save failure")):
