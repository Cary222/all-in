...
from allin.collection.base import CollectionBlockedError, CollectorHooks
...
from allin.collection.platforms.job51 import (
    API_PAGE_SIZE,
    HARD_MAX_PAGES,
    Job51Browser,
    Job51Collector,
...
class Job51CitySnapshotTests(TestCase):
    ...
    def test_city_snapshot_loads(self):
        ...
        self.assertEqual(snap["schema"], "allin.51job_cities.v1")
...
class CollectionFlowSafetyTests(TestCase):
    ...
    def test_probe_risk_signal_stops_without_marking_keyword_complete(self):
        ...
        with (
            patch("allin.collection.platforms.job51.get_page_progress", return_value=0),
            patch.object(Job51Collector, "_plan_probe_pages", return_value=[1]),
            patch.object(collector, "_fetch_page", return_value=blocked),
            patch("allin.collection.platforms.job51.mark_combo_collected") as mark_complete,
        ):
    ...
    def test_hot_pages_are_checkpointed_in_ascending_order(self):
        ...
        with (
            patch("allin.collection.platforms.job51.get_page_progress", return_value=0),
            patch("allin.collection.platforms.job51.upsert_page_progress",
                  side_effect=lambda _conn, _source, _city, _keyword, page: checkpoints.append(page)),
            patch("allin.collection.platforms.job51.mark_combo_collected") as mark_complete,
            patch("allin.collection.platforms.job51.delete_page_progress"),
            patch("allin.collection.platforms.job51._wait_or_stop", return_value=False),
    ...
    def test_collect_only_closes_temporary_host_tab(self):
        ...
        for owned in (False, True):
            with self.subTest(owned=owned):
                ...
                with (
                    patch("allin.collection.platforms.job51.SendWindowChecker.is_active", return_value=True),
                    patch("allin.collection.platforms.job51.should_take_day_off", return_value=False),
...
class CollectEntryTests(TestCase):
    ...
    def test_outside_send_window_returns_completed(self):
        ...
        with patch("allin.collection.platforms.job51.SendWindowChecker.is_active", return_value=False):
            result = collector.collect(request, self._hooks())
        self.assertEqual(result.status, "completed")
    ...
    def test_day_off_returns_completed(self):
        ...
        with (
            patch("allin.collection.platforms.job51.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.job51.should_take_day_off", return_value=True),
        ):
            result = collector.collect(request, self._hooks())
        self.assertEqual(result.status, "completed")
...
class WaitOrStopTests(TestCase):
    ...
    def test_sleeps_when_no_stop_event(self):
        from allin.collection.platforms.job51 import _wait_or_stop
        slept = []
        with patch("allin.collection.platforms.job51.time.sleep", side_effect=slept.append):
