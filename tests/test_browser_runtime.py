...
class BrowserRuntimeManagerTests(unittest.TestCase):
    def test_get_runtime_url_uses_configured_host_and_port(self):
        from allin.browser.runtime import get_runtime_url
    ...
    def test_runtime_script_path_points_to_bundled_proxy(self):
        from allin.browser.runtime import get_runtime_script_path
    ...
    @patch("allin.browser.runtime.subprocess.run")
    @patch("allin.browser.runtime._candidate_node_executables")
    def test_check_node_available_returns_version(self, candidates, run):
        from allin.browser.runtime import check_node_available
    ...
    @patch("allin.browser.runtime.subprocess.run")
    @patch("allin.browser.runtime.Path.is_file", return_value=True)
    @patch("allin.browser.runtime._candidate_node_executables")
    ...
    @patch("allin.browser.runtime.httpx.get")
    def test_runtime_targets_returns_array_from_allin_runtime(self, http_get):
        from allin.browser.runtime import runtime_targets

        health = Mock(status_code=200)
        health.json.return_value = {"status": "ok", "runtime": "allin"}
    ...
    @patch("allin.browser.runtime.httpx.get")
    def test_runtime_targets_rejects_non_allin_runtime(self, http_get):
        from allin.browser.runtime import runtime_targets
    ...
    @patch("allin.browser.runtime.runtime_targets")
    @patch("allin.browser.runtime.start_runtime")
    @patch("allin.browser.runtime.check_node_available")
    ...
    @patch("allin.browser.runtime.time.sleep")
    @patch("allin.browser.runtime._is_port_available")
    @patch("allin.browser.runtime.runtime_health")
    @patch("allin.browser.runtime.runtime_targets")
    @patch("allin.browser.runtime.start_runtime")
    @patch("allin.browser.runtime.check_node_available")
    def test_ensure_runtime_uses_next_free_port_when_default_has_non_allin_service(
        self,
        check_node,
        start_runtime,
        runtime_targets,
        ...
        from allin.browser.runtime import ensure_runtime, get_runtime_url
    ...
    @patch("allin.browser.runtime.subprocess.Popen")
    def test_start_runtime_sets_allin_environment(self, popen):
        from allin.browser.runtime import start_runtime

        with patch.dict(os.environ, {}, clear=True):
            start_runtime({"browser": {"proxy_port": 4567, "chrome_ports": [9222, 9333], "enable_port_guard": False}})

        env = popen.call_args.kwargs["env"]
        self.assertEqual(env["ALLIN_BROWSER_PROXY_PORT"], "4567")
...
class BrowserRuntimeSourceTests(unittest.TestCase):
    def test_cdp_proxy_fetches_browser_websocket_url_for_fallback_ports(self):
        script = Path(__file__).parents[1] / "src" / "allin" / "browser" / "runtime" / "cdp-proxy.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("/json/version", source)
        self.assertIn("webSocketDebuggerUrl", source)

    def test_cdp_proxy_only_reuses_allin_runtime_on_occupied_port(self):
        script = Path(__file__).parents[1] / "src" / "allin" / "browser" / "runtime" / "cdp-proxy.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("const RUNTIME_NAME = 'allin'", source)
    ...
    def test_check_runtime_requires_allin_runtime_identity(self):
        script = Path(__file__).parents[1] / "src" / "allin" / "browser" / "runtime" / "check-runtime.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("/health", source)
        self.assertIn("runtime === 'allin'", source)
