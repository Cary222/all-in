...
from allin.web.server import _execute_collect
from allin.web.tasks import WorkbenchTask
from allin.collection.models import JobCandidate, PlatformCollectionResult
from allin.collection.orchestrator import CollectionOrchestrator
...
class ScraperBackgroundTests(unittest.TestCase):
    def test_collection_options_report_automatic_scoring_to_workbench(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("allin.web.server.CollectionOrchestrator", side_effect=orchestrator), \
                 patch("allin.ai.scorer.score_jobs", side_effect=score_with_progress) as score:
    ...
    def test_stopped_collection_does_not_open_a_search_page(self):
        ...
        with patch("allin.scraper.jobs.get_db", return_value=db), \
             patch("allin.scraper.jobs.new_tab") as new_tab:
    ...
    def test_workbench_passes_its_stop_event_into_collection(self):
        ...
        with patch("allin.scraper.jobs.scrape_jobs", side_effect=scrape_with_progress) as scrape, \
             patch("allin.ai.scorer.score_jobs", side_effect=score_with_progress):
            _execute_collect(task, config)
    ...
    def test_scraper_reports_seen_new_and_duplicate_counts(self):
        ...
        with patch("allin.scraper.jobs.get_db", return_value=db), \
             patch("allin.collection.platforms.boss.PlatformAccessGuard") as guard_cls, \
             patch("allin.scraper.jobs.Progress", return_value=progress_context), \
             patch("allin.scraper.jobs.PageThrottle") as throttle_cls, \
             patch("allin.scraper.jobs.new_tab", return_value="worker-target"), \
             patch("allin.scraper.jobs.navigate", return_value=True), \
             patch("allin.scraper.jobs.evaluate", side_effect=[
                 json.dumps({"risk": None}), False, json.dumps(jobs),
                 json.dumps({"risk": None}), json.dumps(detail),
             ]), \
             patch("allin.scraper.jobs.wait_for_load"), \
             patch("allin.scraper.jobs.scroll"), \
             patch("allin.scraper.jobs.close_tab"), \
             patch("allin.scraper.jobs.job_exists", side_effect=[True, False]), \
             patch("allin.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("allin.scraper.jobs.insert_job"), \
             patch("allin.scraper.jobs.time.sleep"):
    ...
    def test_search_and_detail_pages_reuse_one_background_worker_tab(self):
        ...
        with patch("allin.scraper.jobs.get_db", return_value=db), \
             ...
             patch("allin.scraper.jobs.PageThrottle") as throttle_cls, \
             patch(
                 "allin.scraper.jobs.new_tab",
                 return_value="worker-target",
             ) as new_tab, \
             patch("allin.scraper.jobs.navigate", return_value=True) as navigate, \
             patch(
                 "allin.scraper.jobs.evaluate",
                 side_effect=[
                     json.dumps({"risk": None}), False, json.dumps(jobs),
                     json.dumps({"risk": None}), json.dumps(detail),
                 ],
             ...
             patch("allin.scraper.jobs.scroll"), \
             patch("allin.scraper.jobs.close_tab"), \
             patch("allin.scraper.jobs.job_exists", return_value=False), \
             patch("allin.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("allin.scraper.jobs.insert_job"), \
             patch("allin.scraper.jobs.time.sleep"):
            throttle_cls.return_value.wait.return_value = None
