...
from allin.web.preflight import check_ai_connection, check_browser_connection, collect_preflight_checks
...
class AiPreflightTests(unittest.TestCase):
	...
	@patch("allin.web.preflight.httpx.get")
	...
	def test_ai_client_initialization_error_is_reported_as_runtime_failure(self, http_get):
		...
		self.assertIn("All In", checks[0]["detail"])
		self.assertNotIn(api_key, str(checks))
		self.assertNotIn(base_url, str(checks))

	@patch("allin.web.preflight.sys.version", "3.13.0a5 (test build)")
	@patch("allin.web.preflight.sys.version_info", Mock(releaselevel="alpha"))
	@patch("allin.web.preflight.httpx.get")
...
class BrowserPreflightTests(unittest.TestCase):
	@patch("allin.web.preflight.run_browser_diagnostics")
	...
	@patch("allin.web.preflight.check_ai_connection")
	@patch("allin.web.preflight.check_browser_connection")
	def test_full_flow_uses_only_explicit_full_flow_platform_order(self, browser_check, ai_check):
		...
		self.assertIn("已选择平台均支持自动投递", platform_check["detail"])
