...
from allin.cities import CityRefreshError, load_city_snapshot, refresh_city_cache
from allin.db import get_db, insert_job, update_job_status
from allin.job_export import InvalidJobSelectionError, build_csv, build_xlsx, export_jobs
...
def test_bundled_city_snapshot_is_complete_and_offline(tmp_path):
	with patch("allin.cities.httpx.get") as http_get:
...
def test_html_city_refresh_does_not_overwrite_cache(tmp_path):
	...
	assert json.loads(cache_path.read_text(encoding="utf-8"))["schema"] == "allin.cities.v1"
