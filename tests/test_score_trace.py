...
from allin.ai import scorer
from allin.ai.scorer import ScoreOutcome
...
def test_successful_score_persists_trace_while_prefilter_and_failure_do_not(tmp_path):
    ...
    with (
        patch("allin.ai.scorer.get_db", side_effect=lambda: get_db(db_path)),
        patch("allin.ai.scorer._load_resume", return_value="resume"),
        patch("allin.ai.scorer.quick_score", side_effect=quick_score),
        patch("allin.ai.scorer._score_job_with_ai", side_effect=outcomes),
    ):
...
def test_prefilter_and_failed_rescore_preserve_existing_valid_trace(tmp_path):
    ...
    with (
        patch("allin.ai.scorer.get_db", side_effect=lambda: get_db(db_path)),
        patch("allin.ai.scorer._load_resume", return_value="resume"),
        patch("allin.ai.scorer.quick_score", return_value=(0, "硬性条件不符")),
    ):
    ...
    with (
        ...
        patch(
            "allin.ai.scorer._score_job_with_ai",
