...
from allin.ai import credentials, greeter, scorer
...
class AiCredentialErrorTests(unittest.TestCase):
    def test_openai_compatible_quota_error_is_not_silently_swallowed(self):
        ...
        with patch("allin.ai.credentials.httpx.post", return_value=response):
...
class ScorerTokenResilienceTests(unittest.TestCase):
    ...
    def test_alias_response_scores_successfully_end_to_end(self):
        ...
        with (
            patch("allin.ai.scorer.get_db", return_value=db),
            patch("allin.ai.scorer._load_resume", return_value="真实简历"),
            patch("allin.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("allin.ai.scorer._call_claude", return_value=response),
            patch("allin.ai.scorer.update_job_quick_score"),
            patch("allin.ai.scorer.update_job_score"),
            patch("allin.ai.scorer.update_job_status"),
        ):
    ...
    def test_request_score_reports_specific_validation_failure(self):
        ...
        with (
            patch("allin.ai.scorer._call_claude", return_value="模型输出的不是 JSON"),
            patch("allin.ai.scorer._notify"),
        ):
    ...
    def test_borderline_structured_score_is_reviewed_and_averaged(self):
        ...
        with (
            ...
            patch("allin.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("allin.ai.scorer._call_claude", side_effect=[first, review]) as call_ai,
            patch("allin.ai.scorer.update_job_quick_score"),
            patch("allin.ai.scorer.persist_job_score_and_trace") as persist_score,
            patch("allin.ai.scorer.update_job_status"),
        ):
            scored, filtered = scorer.score_jobs(
                {
    ...
    def test_ai_calls_run_with_configured_concurrency_but_db_writes_stay_on_main_thread(self):
        ...
        with (
            patch("allin.ai.scorer.get_db", return_value=db),
            patch("allin.ai.scorer._load_resume", return_value="真实简历"),
            patch("allin.ai.scorer.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("allin.ai.scorer._call_claude", side_effect=call_ai),
            patch("allin.ai.scorer.update_job_quick_score", side_effect=record_write),
            patch("allin.ai.scorer.update_job_score", side_effect=record_write),
            patch("allin.ai.scorer.update_job_status", side_effect=record_write),
        ):
            scored, filtered = scorer.score_jobs(
    ...
    def test_stop_returns_without_waiting_for_inflight_concurrent_ai_calls(self):
        ...
        with (
            patch("allin.ai.scorer.get_db", return_value=db),
            patch("allin.ai.scorer._load_resume", return_value="真实简历"),
            patch("allin.ai.scorer.get_jobs_by_status", return_value=[_job("1"), _job("2")]),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("allin.ai.scorer._call_claude", side_effect=blocking_ai),
            patch("allin.ai.scorer.update_job_quick_score"),
            patch("allin.ai.scorer.update_job_score"),
            patch("allin.ai.scorer.update_job_status"),
        ):
    ...
    def test_scoring_processes_all_unscored_pending_jobs(self):
        ...
        with (
            patch("allin.ai.scorer.get_db", return_value=db),
            patch("allin.ai.scorer._load_resume", return_value="真实简历"),
            patch("allin.ai.scorer.get_jobs_by_status", return_value=[old_job, new_job]),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "allin.ai.scorer._call_claude",
                return_value=_score_response(82),
            ) as call_ai,
            patch("allin.ai.scorer.update_job_quick_score") as update_quick_score,
            patch("allin.ai.scorer.update_job_score"),
            patch("allin.ai.scorer.update_job_status"),
        ):
            scored, filtered = scorer.score_jobs(
    ...
    def test_context_limit_retry_preserves_full_resume_and_jd(self):
        ...
        with (
            patch("allin.ai.scorer.get_db", return_value=db),
            patch("allin.ai.scorer._load_resume", return_value=resume),
            patch("allin.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("allin.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "allin.ai.scorer._call_claude",
    ...
    def test_quota_error_pauses_without_changing_pending_jobs(self):
        ...
        with (
            ...
            patch("allin.ai.scorer.update_job_quick_score"),
            patch("allin.ai.scorer.update_job_score"),
            patch("allin.ai.scorer.update_job_status") as update_status,
        ):
            scored, filtered = scorer.score_jobs(
                {
                    "scoring": {"threshold": 70},
    ...
    def test_persistent_empty_response_fails_single_job_and_continues_batch(self):
        ...
        with (
            ...
            patch("allin.ai.scorer.update_job_quick_score"),
            patch("allin.ai.scorer.update_job_score") as update_score,
            patch("allin.ai.scorer.update_job_status") as update_status,
            patch("allin.ai.scorer.persist_job_score_and_trace") as persist_score,
        ):
            scored, filtered = scorer.score_jobs(
    ...
    def test_deleted_job_mid_batch_is_skipped_and_batch_continues(self):
        ...
        with (
            ...
            patch("allin.ai.scorer.update_job_status") as update_status,
            patch(
                "allin.ai.scorer.persist_job_score_and_trace",
...
class GreeterTokenResilienceTests(unittest.TestCase):
    ...
    def test_portfolio_is_only_added_when_the_job_explicitly_requests_it(self):
        ...
        with patch("allin.ai.greeter._call_claude", return_value="生成结果") as call_ai:
    ...
    def test_greeting_prompt_forbids_unprovided_urls(self):
        with patch("allin.ai.greeter._call_claude", return_value="普通招呼语") as call_ai:
    ...
    def test_repeated_invented_urls_are_not_saved(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.greeter._get_resume_summary", return_value="不含网址的真实简历"),
            patch(
                "allin.ai.greeter._call_claude",
                side_effect=[
            ...
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_preview,
            patch("allin.ai.greeter.add_history"),
        ):
    ...
    def test_invented_url_never_enters_ready_state_end_to_end(self):
        ...
        from allin.db import get_db as open_db
        from allin.db import insert_job, update_job_status
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("allin.ai.greeter.get_db", side_effect=lambda *args, **kwargs: open_db(db_path)),
                patch("allin.ai.greeter._get_resume_summary", return_value="这份简历摘要不含任何网址"),
                patch(
                    "allin.ai.greeter._call_claude",
                    return_value="项目介绍：https://invented.example/project",
                ),
    ...
    def test_service_level_ai_error_records_pause_reason_in_report(self):
        ...
        from allin.ai.credentials import AIRequestError
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.greeter._get_resume_summary", return_value="真实简历摘要，不含网址"),
            patch(
                "allin.ai.greeter._call_claude",
                side_effect=AIRequestError("quota", "AI 账户额度不足", status_code=402),
            ),
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as update_greeting,
            patch("allin.ai.greeter.add_history"),
        ):
    ...
    def test_missing_resume_records_pause_reason(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=[_job("no-resume")]),
            patch("allin.ai.greeter._get_resume_summary", return_value=""),
        ):
            count = greeter.generate_greetings(config)
    ...
    def test_preserve_failure_reports_conflict(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=[existing]),
            patch("allin.ai.greeter.mark_existing_greeting_ready", return_value=False) as mark,
            patch("allin.ai.greeter._notify"),
        ):
            count = greeter.generate_greetings(config)
    ...
    def test_empty_job_ids_list_processes_nothing(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status") as by_status,
        ):
    ...
    def test_existing_greeting_is_preserved_and_marked_ready(self):
        ...
        with (
            patch("allin.ai.greeter.mark_existing_greeting_ready", return_value=True) as mark_ready,
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=[existing, new_job]),
            patch("allin.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch("allin.ai.greeter._call_claude", return_value="新生成的招呼语") as call_ai,
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_preview,
        ):
            count = greeter.generate_greetings(config)
    ...
    def test_style_guard_rewrites_even_when_model_review_is_malformed(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.greeter._get_resume_summary", return_value="匿名简历摘要"),
            patch(
                "allin.ai.greeter._call_claude",
                side_effect=[
    ...
    def test_style_suggestions_can_be_disabled(self):
        ...
        with (
            ...
            patch("allin.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.greeter._get_resume_summary", return_value="匿名简历摘要"),
            patch("allin.ai.greeter._call_claude", return_value="首次生成的招呼语") as call_ai,
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_preview,
        ):
            count = greeter.generate_greetings({
                "ai": {
    ...
    def test_human_reviewed_greeting_is_not_regenerated(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter.get_jobs_by_status", return_value=[job]),
            patch("allin.ai.greeter._get_resume_summary", return_value="匿名简历摘要"),
            patch("allin.ai.greeter._call_claude") as call_ai,
            patch("allin.ai.greeter.save_generated_greeting_preview") as save_preview,
        ):
            count = greeter.generate_greetings({})
    ...
    def test_empty_greeting_retries_before_leaving_job_pending(self):
        ...
        with (
            ...
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_preview,
            patch("allin.ai.greeter.add_history") as add_history,
        ):
            count = greeter.generate_greetings(
                {
                    "ai": {
    ...
    def test_truncated_greeting_retries_with_larger_output_limit(self):
        ...
        with (
            ...
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True),
        ):
            count = greeter.generate_greetings(
                {
                    "ai": {"greeting_max_iterations": 0},
    ...
    def test_generated_greeting_state_conflict_is_not_counted_or_saved(self):
        ...
        with (
            ...
            patch("allin.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("allin.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch("allin.ai.greeter._call_claude", return_value="已生成但不应保存的招呼语"),
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=False) as save_greeting,
        ):
            count = greeter.generate_greetings(config)
    ...
    def test_explicit_regeneration_replaces_only_the_loaded_greeting_snapshot(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch("allin.ai.greeter._call_claude", return_value="重新生成的招呼语") as call_ai,
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_greeting,
            patch("allin.ai.greeter.mark_existing_greeting_ready") as mark_ready,
        ):
    ...
    def test_job_ids_filter_excludes_scored_status_before_ai_call(self):
        ...
        with (
            patch("allin.ai.greeter.get_db", return_value=db),
            patch("allin.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch("allin.ai.greeter._call_claude", return_value="只为允许状态生成") as call_ai,
            patch("allin.ai.greeter.save_generated_greeting_preview", return_value=True) as save_greeting,
        ):
