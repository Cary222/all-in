...
from allin.config import DEFAULTS, load_config
...
from allin.web.server import _execute_collect, _wait_for_collection_delivery_cooldown
...
""",
    ...
    def test_legacy_global_safety_lock_migrates_to_boss_platform(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "allin.db"
            ...
            legacy.execute(
                "INSERT INTO platform_safety_state (id, reason, locked_until) VALUES (1, 'captcha', datetime('now', '+30 minutes'))"
            )
            legacy.commit()
            legacy.close()
            ...
            try:
                columns = {row[1] for row in migrated.execute("PRAGMA table_info(platform_safety_state)")}
                lock = get_active_platform_safety_lock(migrated, platform="boss")
            finally:
                migrated.close()

        self.assertIn("platform", columns)
        self.assertNotIn("id", columns)
        self.assertEqual(lock["platform"], "boss")
        self.assertEqual(lock["reason"], "captcha")
    ...
    def test_collection_risk_stops_and_records_safe_reason(self):
        ...
        with patch("allin.scraper.jobs.get_db", return_value=db), \
             ...
             patch("allin.scraper.jobs.Progress", return_value=context), \
             patch("allin.scraper.jobs.new_tab", return_value="worker"), \
             patch("allin.scraper.jobs.wait_for_load"), \
             patch("allin.scraper.jobs.evaluate", return_value=json.dumps({"risk": "captcha"})), \
    ...
    def test_consecutive_page_failures_end_collection_without_risk_lock(self):
        ...
        with patch("allin.scraper.jobs.get_db", return_value=db), \
             patch("allin.collection.platforms.boss.PlatformAccessGuard") as guard_cls, \
             patch("allin.scraper.jobs.Progress", return_value=context), \
             patch("allin.scraper.jobs.new_tab", return_value=None), \
             patch("allin.scraper.jobs.close_tab"), \
             patch("allin.scraper.jobs.time.sleep"):
            count = scrape_jobs(config, ["AI"])
    ...
    def test_frontend_task_log_explains_daily_limit(self):
        ...
        with patch("allin.scraper.jobs.scrape_jobs", side_effect=fake_scrape), \
             patch("allin.ai.scorer.score_jobs", return_value=(0, 0)):
    ...
    def test_collection_delivery_cooldown_selects_one_random_value_per_flow(self):
        ...
        with patch("allin.web.server.random.uniform", return_value=11.25) as choose:
