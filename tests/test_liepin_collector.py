...
from allin.collection.base import CollectorHooks
from allin.collection.models import PlatformCollectionRequest
from allin.collection.orchestrator import normalize_collection_options
from allin.collection.platforms.liepin import (
    JS_EXTRACT_DETAIL,
    JS_EXTRACT_LIST,
    LiepinBrowser,
    LiepinCollector,
...
class LiepinCollectorTests(TestCase):
    def setUp(self):
        self._patches = [
            patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.liepin.should_take_day_off", return_value=False),
        ]
...
class LiepinEnhancedTests(TestCase):
    ...
    def test_outside_send_window_skips_collection(self):
        ...
        with patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=False):
    ...
    def test_day_off_skips_collection(self):
        ...
        with (
            patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.liepin.should_take_day_off", return_value=True),
        ):
            result = collector.collect(
                PlatformCollectionRequest("liepin", ["AI"], ["上海"], {"上海": "020"}, max_pages=1),
                self._hooks(),
...
class LiepinResumeCheckpointTests(TestCase):
    ...
    def _resume_patches(self, collected_combos=None, saved_page=0):
        ...
        return (
            patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.liepin.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.liepin.prune_collected_combos"),
            patch("allin.collection.platforms.liepin.prune_page_progress"),
            patch("allin.collection.platforms.liepin.get_collected_combos", return_value=collected_combos),
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=saved_page),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected"),
            patch("allin.collection.platforms.liepin.delete_page_progress"),
        )
    ...
    def test_completed_combo_skipped_entirely(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.liepin.prune_collected_combos"),
            patch("allin.collection.platforms.liepin.prune_page_progress"),
            patch("allin.collection.platforms.liepin.get_collected_combos", return_value={("上海", "AI")}),
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=0),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected"),
            patch("allin.collection.platforms.liepin.delete_page_progress"),
        ):
    ...
    def test_page_progress_resumes_from_saved_page(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.liepin.prune_collected_combos"),
            patch("allin.collection.platforms.liepin.prune_page_progress"),
            patch("allin.collection.platforms.liepin.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=2),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected"),
            patch("allin.collection.platforms.liepin.delete_page_progress"),
        ):
    ...
    def test_word_completion_marks_combo_and_clears_page_progress(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=0),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected") as mark_complete,
            patch("allin.collection.platforms.liepin.delete_page_progress") as delete_progress,
        ):
            result = collector.collect(
                PlatformCollectionRequest("liepin", ["AI"], ["上海"], {"上海": "020"}, max_pages=2),
                self._hooks(collected),
    ...
    def test_saved_page_exceeds_max_pages_skips_keyword(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.liepin.prune_page_progress"),
            patch("allin.collection.platforms.liepin.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=5),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected") as mark_complete,
            patch("allin.collection.platforms.liepin.delete_page_progress") as delete_progress,
        ):
    ...
    def test_prune_called_on_collect_start(self):
        ...
        with (
            patch("allin.collection.platforms.liepin.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.liepin.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.liepin.prune_collected_combos") as prune_combos,
            patch("allin.collection.platforms.liepin.prune_page_progress") as prune_pages,
            patch("allin.collection.platforms.liepin.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.liepin.get_page_progress", return_value=0),
            patch("allin.collection.platforms.liepin.upsert_page_progress"),
            patch("allin.collection.platforms.liepin.mark_combo_collected"),
    ...
    def test_no_safety_conn_skips_resume_logic(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.liepin.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.liepin.prune_collected_combos") as prune_combos,
            patch("allin.collection.platforms.liepin.get_collected_combos") as get_combos,
            patch("allin.collection.platforms.liepin.get_page_progress") as get_progress,
            patch("allin.collection.platforms.liepin.upsert_page_progress") as upsert,
            patch("allin.collection.platforms.liepin.mark_combo_collected") as mark,
        ):
            result = collector.collect(
                PlatformCollectionRequest("liepin", ["AI"], ["上海"], {"上海": "020"}, max_pages=2),
                self._hooks(collected),
