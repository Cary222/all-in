...
from allin.collection.base import CollectionBlockedError, CollectionError, CollectorHooks
from allin.collection.models import PlatformCollectionRequest
from allin.collection.platforms.zhilian import (
    JD_CLASSES,
    JS_EXTRACT_DETAIL,
    JS_EXTRACT_LIST,
    ZhilianBrowser,
...
class ZhilianFixtureTests(TestCase):
    def setUp(self):
        self._patches = [
            patch("allin.collection.platforms.zhilian.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.zhilian.should_take_day_off", return_value=False),
        ]
    ...
    def test_city_snapshot_is_local_and_not_shared_with_boss_codes(self):
        ...
        self.assertEqual(snapshot["schema"], "allin.zhilian_cities.v1")
...
class ZhilianEnhancedTests(TestCase):
    ...
    def test_outside_send_window_skips_collection(self):
        ...
        with patch("allin.collection.platforms.zhilian.SendWindowChecker.is_active", return_value=False):
    ...
    def test_day_off_skips_collection(self):
        ...
        with (
            patch("allin.collection.platforms.zhilian.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.zhilian.should_take_day_off", return_value=True),
        ):
            result = collector.collect(
                PlatformCollectionRequest("zhilian", ["AI"], ["北京"], {"北京": "530"}, max_pages=1),
                self._hooks(),
    ...
    def test_deal_breaker_filter(self):
        from allin.collection.models import JobCandidate
...
class ZhilianResumeCheckpointTests(TestCase):
    ...
    def test_completed_combo_skipped_entirely(self):
        ...
        with (
            patch("allin.collection.platforms.zhilian.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.zhilian.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.zhilian.prune_collected_combos"),
            patch("allin.collection.platforms.zhilian.prune_page_progress"),
            patch("allin.collection.platforms.zhilian.get_collected_combos", return_value={("北京", "AI")}),
            patch("allin.collection.platforms.zhilian.get_page_progress", return_value=0),
            patch("allin.collection.platforms.zhilian.upsert_page_progress"),
            patch("allin.collection.platforms.zhilian.mark_combo_collected"),
            patch("allin.collection.platforms.zhilian.delete_page_progress"),
        ):
            result = collector.collect(
                PlatformCollectionRequest("zhilian", ["AI"], ["北京"], {"北京": "530"}, max_pages=1),
    ...
    def test_pages_are_checkpointed_in_ascending_order(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.zhilian.prune_collected_combos"),
            patch("allin.collection.platforms.zhilian.prune_page_progress"),
            patch("allin.collection.platforms.zhilian.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.zhilian.get_page_progress", return_value=0),
            patch("allin.collection.platforms.zhilian.upsert_page_progress",
    ...
    def test_word_completion_marks_combo_and_clears_page_progress(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.zhilian.get_page_progress", return_value=0),
            patch("allin.collection.platforms.zhilian.upsert_page_progress"),
            patch("allin.collection.platforms.zhilian.mark_combo_collected") as mark_complete,
            patch("allin.collection.platforms.zhilian.delete_page_progress") as delete_progress,
        ):
            result = collector.collect(
    ...
    def test_saved_page_exceeds_max_pages_skips_keyword(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.zhilian.prune_page_progress"),
            patch("allin.collection.platforms.zhilian.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.zhilian.get_page_progress", return_value=5),
            patch("allin.collection.platforms.zhilian.upsert_page_progress"),
            patch("allin.collection.platforms.zhilian.mark_combo_collected") as mark_complete,
            patch("allin.collection.platforms.zhilian.delete_page_progress") as delete_progress,
        ):
    ...
    def test_prune_called_on_collect_start(self):
        ...
        with (
            patch("allin.collection.platforms.zhilian.SendWindowChecker.is_active", return_value=True),
            patch("allin.collection.platforms.zhilian.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.zhilian.prune_collected_combos") as prune_combos,
            patch("allin.collection.platforms.zhilian.prune_page_progress") as prune_pages,
            patch("allin.collection.platforms.zhilian.get_collected_combos", return_value=set()),
            patch("allin.collection.platforms.zhilian.get_page_progress", return_value=0),
            patch("allin.collection.platforms.zhilian.upsert_page_progress"),
            patch("allin.collection.platforms.zhilian.mark_combo_collected"),
    ...
    def test_no_safety_conn_skips_resume_logic(self):
        ...
        with (
            ...
            patch("allin.collection.platforms.zhilian.should_take_day_off", return_value=False),
            patch("allin.collection.platforms.zhilian.prune_collected_combos") as prune_combos,
            patch("allin.collection.platforms.zhilian.get_collected_combos") as get_combos,
            patch("allin.collection.platforms.zhilian.get_page_progress") as get_progress,
            patch("allin.collection.platforms.zhilian.upsert_page_progress") as upsert,
            patch("allin.collection.platforms.zhilian.mark_combo_collected") as mark,
        ):
            result = collector.collect(
                PlatformCollectionRequest("zhilian", ["AI"], ["北京"], {"北京": "530"}, max_pages=2),
                self._hooks(collected),
...
class ZhilianApiRateLimiterTests(TestCase):
    ...
    def test_wait_before_request_returns_true_no_stop(self):
        ...
        with patch("allin.collection.platforms.zhilian._wait_or_stop", return_value=False):
