...
class ResumeArtifactTests(unittest.TestCase):
    def test_prompt_forbids_resume_tailoring_artifacts(self):
        from allin.ai.resume import RESUME_TAILOR_PROMPT
    ...
    def test_finds_resume_artifact_phrases(self):
        from allin.ai.resume import _find_resume_artifacts
    ...
    def test_existing_source_placeholders_are_allowed_but_new_or_rewritten_ones_are_blocked(self):
        from allin.ai.resume import _find_new_placeholders
    ...
    def test_integrity_checks_report_new_and_missing_fact_values(self):
        from allin.ai.resume import _find_blocking_integrity_issues
    ...
    @patch("allin.ai.resume._render_pdf")
    @patch("allin.ai.resume._call_claude")
    @patch("allin.ai.resume.get_db")
    def test_blocking_validation_failure_is_returned_to_monitor(self, get_db, call_claude, render_pdf):
        from allin.ai.resume import (
    ...
    def test_completed_resume_marker_is_removed_before_writing(self, get_db, call_claude, render_pdf):
        from allin.ai.resume import RESUME_COMPLETION_MARKER, generate_tailored_resume
...
class ResumePdfRuntimeTests(unittest.TestCase):
    @patch("allin.ai.resume.wait_for_load", return_value=True)
    @patch("allin.ai.resume.close_tab")
    @patch("allin.ai.resume.print_pdf")
    @patch("allin.ai.resume.new_tab")
    def test_render_pdf_via_cdp_uses_browser_facade(self, new_tab, print_pdf, close_tab, wait_for_load):
        from allin.ai.resume import _render_pdf_via_cdp
    ...
    @patch("allin.ai.resume.close_tab")
    @patch("allin.ai.resume.print_pdf")
    @patch("allin.ai.resume.new_tab", return_value="target-1")
    ...
    @patch("allin.ai.resume._render_pdf_via_cdp", return_value=False)
    def test_render_pdf_does_not_emit_fontless_fallback_pdf(self, render_via_cdp):
        from allin.ai.resume import _render_pdf
