...
from allin.executor.sender import CHAT_BUTTON_SCRIPT_FOR_TESTS
from allin.executor.sender import _chat_target_matches_job
from allin.executor.sender import _confirm_preset_greeting
from allin.executor.sender import _detect_greet_popup
from allin.executor.sender import _handle_greet_popup
from allin.executor.sender import _message_delivery_state
from allin.executor.sender import _verify_greeting_in_chat_list
from allin.executor.sender import send_greetings
from allin.executor.sender import _send_greeting_once
from allin.executor.sender import _submit_chat_message_background
from allin.executor.sender import _submit_startchat_greeting
from allin.executor.sender import _wait_for_chat_page
...
class JobSelectionTests(unittest.TestCase):
    ...
    def test_preset_confirmation_uses_background_dom_click(self):
        with patch(
            "allin.executor.sender.evaluate",
            return_value='{"success": true, "action": "preset_confirmed"}',
        ) as evaluate_mock, patch("allin.executor.sender.click_at") as click_at:
    ...
    def test_startchat_submission_uses_trusted_typing_and_real_send_message_click(self):
        ...
        with patch(
            ...
            "allin.executor.sender.click_at",
            return_value=True,
        ) as click_at, patch(
            "allin.executor.sender.press_key",
            return_value=True,
        ) as press_key, patch(
            "allin.executor.sender.type_text",
            return_value=True,
        ) as type_text:
    ...
    def test_chat_list_verification_requires_company_and_complete_greeting(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="chat-target"), \
             patch("allin.executor.sender.wait_for_load"), \
             patch("allin.executor.sender.evaluate", return_value=result) as evaluate, \
             patch("allin.executor.sender.close_tab") as close_tab:
    ...
    def test_startchat_popup_reuses_chat_redirect_without_foreground_input(self):
        ...
        with patch(
            "allin.executor.sender._detect_greet_popup",
            return_value={"success": True, "popup": True, "kind": "startchat_dialog"},
        ), patch(
            "allin.executor.sender._navigate_to_chat_redirect",
            return_value=True,
        ) as redirect, patch(
            "allin.executor.sender._submit_startchat_greeting",
    ...
    def test_continue_chat_popup_is_detected_and_confirmed(self):
        with patch(
            "allin.executor.sender.evaluate",
            return_value='{"success": true, "popup": true, "kind": "continue_chat_dialog"}',
        ) as evaluate_mock:
            state = _detect_greet_popup("target-1")

        self.assertEqual(state["kind"], "continue_chat_dialog")
        detection_script = evaluate_mock.call_args.args[1]
        self.assertIn("您与该Boss已沟通过", detection_script)
        self.assertIn("是否就新职位", detection_script)

        with patch(
            "allin.executor.sender._detect_greet_popup",
            return_value={"success": True, "popup": True, "kind": "continue_chat_dialog"},
        ), patch(
            "allin.executor.sender._confirm_continue_chat",
            return_value={"success": True, "action": "continue_chat_confirmed"},
        ) as confirm:
            result = _handle_greet_popup("target-1", "不用重复发送的招呼语")

        self.assertEqual(result["action"], "continue_chat_confirmed")
        confirm.assert_called_once_with("target-1")

    def test_continue_chat_confirmation_is_recorded_as_follow_up_not_send_error(self):
        job = {
            "id": "continued-job",
            "company": "Example",
            "title": "Frontend Engineer",
            "url": "https://www.zhipin.com/job_detail/continued-job.html",
        }

        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             patch("allin.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("allin.executor.sender._click_chat_button", return_value={"success": True}), \
             patch("allin.executor.sender._handle_greet_popup", return_value={"success": True, "action": "continue_chat_confirmed"}), \
             patch("allin.executor.sender._wait_for_chat_page", return_value={"success": False, "error": "chat_navigation_timeout"}), \
             patch("allin.executor.sender._submit_chat_message_background") as submit, \
             patch("allin.executor.sender._fill_chat_input") as fill_input, \
             patch("allin.executor.sender.close_tab") as close_tab, \
             patch("allin.executor.sender.time.sleep"):
        ...
        self.assertTrue(result["continued_existing_conversation"])
        self.assertTrue(result["verified_from_continue_prompt"])
        self.assertIn("继续跟进", result["history_detail"])
        submit.assert_not_called()
        fill_input.assert_not_called()
        close_tab.assert_called_once_with("target-1")

    def test_company_only_chat_match_is_enabled_only_for_continue_chat(self):
        job = {"id": "continued-job", "company": "Example", "title": "New Position"}
        with patch(
            "allin.executor.sender.evaluate",
            return_value='{"success": true, "matches": true}',
        ) as evaluate_mock:
            self.assertTrue(_chat_target_matches_job("target-1", job, allow_company_only=True))

        matching_script = evaluate_mock.call_args.args[1]
        self.assertIn("const allowCompanyOnly = true", matching_script)
    ...
    def test_send_greeting_reports_unavailable_job_page_before_clicking_chat(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1") as new_tab, \
             patch("allin.executor.sender.evaluate", return_value='{"success": false, "error": "job_page_unavailable", "history_detail": "岗位页面不存在或已下架", "skip_backoff": true}'), \
             patch("allin.executor.sender.close_tab") as close_tab, \
             patch("allin.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_uses_real_click_fallback_when_chat_button_does_not_navigate(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             patch("allin.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("allin.executor.sender._click_chat_button", return_value={"success": True, "button_text": "继续沟通"}), \
             patch("allin.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("allin.executor.sender._wait_for_chat_page", side_effect=[
                 {"success": False, "error": "chat_navigation_timeout"},
                 {"success": True, "target_id": "target-1"},
             ]), \
             patch("allin.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("allin.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("allin.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("allin.executor.sender.click_at", return_value=True) as click_at, \
             patch("allin.executor.sender.close_tab") as close_tab, \
             patch("allin.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_waits_for_chat_button_before_failing(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             patch("allin.executor.sender.evaluate", side_effect=evaluate_results) as evaluate_mock, \
             patch("allin.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("allin.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("allin.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("allin.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("allin.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("allin.executor.sender.click_at", return_value=True), \
             patch("allin.executor.sender.close_tab") as close_tab, \
             patch("allin.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_accepts_chat_list_receipt_when_echo_is_missing(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch("allin.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("allin.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("allin.executor.sender._message_delivery_state", return_value="missing"), \
             patch("allin.executor.sender._submit_chat_message_background", return_value={"success": True}), \
             patch("allin.executor.sender._verify_greeting_in_chat_list", return_value=True), \
             patch("allin.executor.sender.close_tab") as close_tab, \
             patch("allin.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_uses_original_flow_when_platform_preset_is_enabled(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch(
                 "allin.executor.sender._confirm_preset_greeting",
                 return_value={"success": True, "action": "preset_confirmed"},
             ) as confirm_preset, \
             patch(
                 "allin.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ), \
             patch(
                 "allin.executor.sender._message_delivery_state",
                 side_effect=["missing", "delivered", "delivered"],
             ), \
             patch(
                 "allin.executor.sender._fill_chat_input",
                 return_value={"success": True, "disabled": False},
             ) as fill_input, \
             patch(
                 "allin.executor.sender._submit_chat_message_background",
                 return_value={"success": True, "action": "chat_submitted_background"},
    ...
    def test_send_greeting_redirects_first_contact_and_uses_background_chat_submit(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             patch("allin.executor.sender.evaluate", return_value='{"success": true}'), \
             patch(
                 "allin.executor.sender._click_chat_button",
                 return_value={
                     "success": True,
             ...
             patch("allin.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch(
                 "allin.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ), \
    ...
    def test_first_contact_without_chat_redirect_fails_without_foreground_click(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch("allin.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch("allin.executor.sender.click_at") as foreground_click, \
             patch("allin.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
                "您好，我对这个岗位很感兴趣。",
    ...
    def test_wait_for_chat_page_rejects_a_different_job_conversation(self):
        ...
        with patch("allin.executor.sender.evaluate", return_value="/web/geek/chat"), \
             patch("allin.executor.sender._chat_target_matches_job", return_value=False) as matches_job, \
    ...
    def test_wait_for_chat_page_adopts_preexisting_chat_when_active_job_matches(self):
        ...
        with patch("allin.executor.sender.evaluate", return_value="/job_detail/expected-job"), \
             patch("allin.executor.sender._chat_target_matches_job", return_value=True) as matches_job, \
             patch("allin.executor.sender.get_page_targets", return_value=targets), \
             patch("allin.executor.sender.time.sleep"):
    ...
    def test_send_greeting_adopts_matching_new_chat_tab_and_closes_old_tab(self):
        ...
        with patch("allin.executor.sender.new_tab", return_value="job-target"), \
             patch("allin.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("allin.executor.sender._click_chat_button", return_value={"success": True}), \
             patch("allin.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch(
    ...
    def test_send_greetings_reopens_job_page_once_when_chat_input_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("allin.db.DB_PATH", db_path), \
                 ...
                 patch("allin.executor.sender._send_greeting_once", side_effect=attempts) as send_once, \
                 patch("allin.executor.sender.close_tab") as close_tab:
    ...
    def test_send_greetings_closes_failed_task_page_after_recording_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("allin.db.DB_PATH", db_path), \
                 ...
                 patch("allin.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch(
                     "allin.executor.sender._send_greeting_once",
    ...
    def test_send_greetings_reports_current_and_next_job_to_workbench(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("allin.db.DB_PATH", db_path), \
                 patch("allin.executor.sender.should_take_day_off", return_value=False), \
                 patch("allin.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch("allin.executor.sender._send_greeting_once", return_value=({"success": True}, None)):
                sent = send_greetings(config, force=True)
    ...
    def test_send_greetings_force_bypasses_send_window_restriction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Act
            with patch("allin.db.DB_PATH", db_path), \
                 patch("allin.throttle.datetime") as mock_datetime, \
                 patch("allin.executor.sender._send_greeting_once", return_value=({"success": True}, None)):
