...
class BrowserDiagnosticsTests(unittest.TestCase):
    ...
    @patch("allin.browser.diagnostics.find_boss_tab")
    @patch("allin.browser.diagnostics.runtime_targets")
    @patch("allin.browser.diagnostics.ensure_runtime")
    @patch("allin.browser.diagnostics.check_node_available")
    ...
    @patch("allin.browser.diagnostics.find_boss_tab")
    @patch("allin.browser.diagnostics.runtime_targets")
    @patch("allin.browser.diagnostics.runtime_health")
    @patch("allin.browser.diagnostics.ensure_runtime")
    @patch("allin.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_updated_url_after_runtime_port_fallback(
        ...
        runtime_health.return_value = {"status": "ok", "runtime": "allin"}
        runtime_targets.return_value = [{"targetId": "1", "url": "https://www.zhipin.com"}]
        find_boss_tab.return_value = {"targetId": "1", "title": "BOSS直聘"}
    ...
    @patch("allin.browser.diagnostics.ensure_runtime")
    @patch("allin.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_non_allin_service_on_runtime_port(self, check_node, ensure_runtime, runtime_health):
        from allin.browser.diagnostics import run_browser_diagnostics
        ...
        self.assertFalse(result["runtime"])
        self.assertTrue(any("non-All In" in error for error in result["errors"]))

    @patch("allin.browser.diagnostics.run_browser_diagnostics")
    def test_print_browser_diagnostics_shows_non_allin_service_message(self, run_browser_diagnostics):
        ...
        run_browser_diagnostics.return_value = {
            ...
            "targets": [],
            "boss_tab": None,
            "errors": ["Runtime port is occupied by a non-All In service."],
        ...
        self.assertIn("端口已被非 All In 服务占用", buffer.getvalue())
