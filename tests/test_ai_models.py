...
from allin.ai.credentials import AIRequestError, list_ai_models
    ...
    def test_model_discovery_paths_auth_and_pagination(self):
        ...
        for service, base, urls, paginated in cases:
            with self.subTest(service=service, base=base), patch.dict("os.environ", {}, clear=True):
                ...
                with patch("allin.ai.credentials.httpx.get", side_effect=get):
    ...
    def test_failures_are_safe_and_empty_list_is_valid(self):
        ...
        for status, payload, kind in [
            ...
            with self.subTest(status=status, payload=payload):
                ...
                with patch("allin.ai.credentials.httpx.get", return_value=response), self.assertRaises(AIRequestError) as error:
                    list_ai_models(config)
                self.assertEqual(error.exception.kind, kind)
                self.assertNotIn("test-secret", str(error.exception))
        with patch("allin.ai.credentials.httpx.get", side_effect=httpx.ReadTimeout("test-secret")), self.assertRaises(AIRequestError) as error:
            list_ai_models(config)
        ...
        with patch("allin.ai.credentials.httpx.get", return_value=response):
            self.assertEqual(list_ai_models(config), [])
        with patch.dict("os.environ", {}, clear=True), patch("allin.ai.credentials.httpx.get") as get:
