...
if ($PythonPath) {
	...
	$RunnerPrefix = @("-m", "allin.main")
} else {
	$AllIn = Get-Command "allin" -ErrorAction SilentlyContinue
	if ($AllIn) {
		$Runner = $AllIn.Source
		$RunnerPrefix = @()
	} else {
	...
	if (-not $Python) {
		throw "Could not find All In or Python. Install the project first with: pip install -e ."
	}
...
Write-Host "All In is ready. Log in manually in the dedicated Chrome window if needed."
