...
多平台智能求职 Agent — 全自动化求职流水线的 Claude Code Skill。
...
用户表示配置完成后：
- 运行 `allin ai-status` 确认 AI API 已连接（不得显示或索要完整 Key）
- 运行 `allin connect` 确认 Chrome 连接正常
- 如果连接正常，提示用户可以开始使用（`allin run` 或分步操作）
...
1. **使用 Google Chrome**：All In 通过 Chrome DevTools Protocol 操作浏览器，不要用 Safari 或其他未连接浏览器替代。
...
4. **连接 AI API**：运行 `allin web`，在本地面板的「AI 设置」中选择服务商、填写 API Key 和模型，然后运行 `allin ai-status`。不得让用户在聊天中发送 Key。
5. **检查浏览器连接**：运行 `allin connect`。该命令只检测连接，不会代替用户启动 Chrome。

只有 AI 和 Chrome 连接都检测通过后，才引导用户运行 `allin run`。
...
如果 `allin` 命令不存在，引导用户执行：
```bash
cd /path/to/all-in && pip install -e .
```
...
| 用户意图 | 命令 |
|---------|------|
| "帮我搜索/采集岗位" | `allin scrape -k "关键词"` |
| "评分/筛选一下" | `allin score` |
| "生成招呼语" | `allin greet` |
| "确认投递" | `allin confirm` |
| "发送" | `allin send` |
| "看看状态/数据" | `allin status --full` |
| "打开看板" | `allin web` |
| "连接/检测 AI API" | 打开本地面板配置，完成后运行 `allin ai-status` |
| "监听回复" | `allin monitor` |
| "生成简历给xx岗位" | `allin resume --job-id xxx` |
...
- SQLite 数据库：`./data/allin.db`
- 定制简历输出：`./data/resumes/`
- 历史记录：`./data/history.jsonl`

## 与 web-access Skill 的关系

All In 依赖 CDP Proxy 进行浏览器操作，其浏览器连接层与 web-access Skill 共享相同的 CDP 连接机制。如果已安装 web-access，两者可共享同一个 Chrome 实例和 Proxy 进程。
